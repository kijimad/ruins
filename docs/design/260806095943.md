...
- [x] Step 1 基盤. `gotext` 依存、`internal/i18n`、config 連動、query.T の実行時解決
- [x] Step 2 抽出と対訳表. 表示される日本語の英語原文起こしと ja.po 対訳を整備
- [x] Step 3 データ. 全 raw が id 正準キー、item/prop/member/tile/profession の name/description を英語原文化、spawn と生成の同定を id 参照へ、日本語を ja.po へ。tile は生成論理キーを name から id へ分離
- [x] Step 4 UI コード. メニュー・HUD・詳細・装備一覧を query.T へ配線
- [x] Step 5 会話とログ. 攻撃・会話・受け渡し・使用・拾得ログの表示名を query.T へ配線
- [~] Step 6 仕上げ. ja.po 完全性ゲート `TestRawTranslationCoverage` をラチェットで実装済み。擬似ローカライズ・ロケール別書式整列・ja/en ゴールデンは未着手
- [x] `make check` 緑

表示される raw の name/description は全て英語原文 + ja.po に揃い、coverage 債務はゼロ。非表示のテーブル名と prop/tile description は表示経路が無く display i18n のスコープ外として日本語のまま残す。
