...
# depot/office/house/store/antique も同様に施設別テーブルへ。全 facilityType を割り当てる
```
...
`urbanEnemyTable = "ruins_area"` は定数のまま、割り当ての無い未知施設のフォールバックに残す。raw.toml へ全 facilityType 分の施設別テーブルを新設する。診療所=医療、研究施設=実験、倉庫=機械害獣、事務所=警備、住宅=生活害獣、商店=害獣と番犬、骨董品店=呪物と怨霊。中身の敵選定は今後の gamedesign 調整対象。

テーブル名はエリアでなく**施設**を表す。粗く束ねると別種の施設が同じ顔ぶれになり納得感が落ちるので、施設ごとにテーマの敵を割り当てる。全施設に割り当てがあることは `TestFacilityEnemyTables_全施設種別を網羅する` で守る。
