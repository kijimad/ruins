...
コードベースを棚卸しした結果を示す。橋タイルの名称定数と歩行可能スポーン仕様のみが実装済みで、橋遷移システムの中核は未着手である。

### タイル

- [x] 橋タイル名の定数 bridge_a〜bridge_d を定義。`internal/consts/consts.go` の TileNameBridgeA〜TileNameBridgeD で確認
- [x] 橋タイルを歩行可能タイルとしてスポーン。`internal/mapspawner/spawner.go` の passableTileSpecs に4種を登録済み
- [ ] 橋タイルの sprite 定義を raw.toml に追加。`assets/metadata/entities/raw/raw.toml` に bridge 系タイルの定義は存在しない。spawnName が解決できず、テンプレートに橋タイルを置いても実体化できない見込み

### エンティティと相互作用

- [ ] BridgeInteraction 構造体と Config メソッドを追加。`internal/components/interactable.go` に該当型はなく、既存の相互作用は InteractionKind の enum 方式で BridgeInteraction は未定義
- [ ] 橋エンティティ定義を raw.toml に追加。entity.bridge_a などの定義は存在しない
- [ ] interaction_activate.go に executeBridge と case を追加。`internal/actions/interaction_activate.go` に executeBridge は存在しない
- [ ] 橋遷移で WarpNextEvent を発行。橋を渡ることを契機とした遷移イベント発行の処理は存在しない。既存の遷移イベントは WarpDescend、WarpAscend、WarpRuinExit、WarpRuinEnter で、橋専用の発行経路はない

### シードと生成

- [ ] Dungeon リソースに NextFloorSeed と SelectedBridgeID を追加。`internal/resources` に両フィールドは存在しない
- [ ] worldhelper に SpawnBridge 関数を追加。`internal/worldhelper/spawn.go` に橋生成関数は存在しない
- [ ] CalculateBridgeSeed 関数を追加。`internal/worldhelper/spawn_bridge.go` 自体が存在せず、シード計算ロジックも未実装

### テンプレート

- [ ] templates/50x50_normal_with_bridges.toml を追加。橋レイアウトのテンプレートは存在しない
- [ ] templates/50x50_rest_with_bridges.toml を追加。休憩階層向けテンプレートは存在しない
- [ ] templates/entities/bridge_entity_*.toml を追加。橋エンティティのテンプレートは存在しない

### 要件の充足状況

- [ ] 各階層の上部に3つ、休憩階層で4つの橋を配置。テンプレートとエンティティが未実装のため未達
- [ ] 橋を渡ると次階層へ一方通行で移動。相互作用とイベント発行が未実装のため未達
- [ ] 橋ごとに異なるシードで次階層を生成。シード計算とリソース保持が未実装のため未達
- [ ] 各階層の下部に到着地点の橋を1つ配置。テンプレート未実装のため未達

### 将来の拡張案

- [~] 橋の状態、ヒント、外観バリエーション、動的な橋数、難易度表示。いずれも本設計で将来の拡張案として明示的に見送り
