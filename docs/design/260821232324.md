...
sequenceDiagram
    ...
    Note over TD: 手当て1回で治療量を負傷度順に配る。浅い傷は複数部位にまたがり、深手は絞られる
```
...
type Wound struct {
	...
	Severity float64 // 深さ 0-1。未治療で上がる
}
...
傷の種類は既存の武器スキルから導く。命中部位は部位サイズの重みで抽選する。傷は毎ターン `WoundSystem` が進める。**軽い傷は手当てせずとも代謝で徐々に閉じる**。悪化するのは深手閾値を超えた重い傷だけで、放置すると `Severity` が上がり出血が増し感染創の感染判定に掛かる。だから掠り傷は戦闘のたびに手当てせずとも自然に癒え、手当てが要るのは重い傷に絞られる。軽い出血も数ターンで自然に止まり、止血が要るのは止まる前に HP を脅かす強い出血だけ。深手まで進んで癒えた傷は `WoundScar` として軽いペナルティを残す。演出はログと敵ステータスに出す。命中時に `左腕を切り裂いた！ 出血` とログし、ステータス欄へ HP と傷と病気を並べる。
...
```go
// Tend は1回の手当て。治療量 budget を負傷度の重い順に配る。部位は選ばない
func Tend(hs *HealthStatus, budget float64)
```

- 治療量 `budget` は薬品の量とグレード、`SkillHealing` から決まる。
- 出血する傷を最優先に、次いで深手から順に処置して `WoundStabilized` にする。1傷の処置に要る量は `Severity` に比例する。
- **浅い傷は少量で済むので、1回の手当てが複数部位にまたがって片付く。深手は量を食うので、`budget` が尽きれば残りは `WoundOpen` のまま次の手当てへ持ち越す。**
...
プレイヤーは「手当て」を選ぶだけで、どこから治すかは負傷度で自動に決まる。傷が多い/深いほど1回で足りず薬が要る。傷ついた→薬で手当て、の最短ループが、良い薬を使うか温存するかの判断とともに回る。全快の安全地帯は作らず、前進の合間に手当てする。治療効果は既存 `ModHealingEffect`・`SkillHealing` を再利用する。
...
| `internal/systems/wound.go` | `WoundSystem` を追加し、軽い傷は代謝で自然治癒させ、深手だけ悪化・傷跡化させ、出血合計で HP を削る |
...
| `internal/activity/`（新規 tend アクション） | `Tend` を追加し、薬を消費して治療量を負傷度順に配る。部位を選ばない |
...
| serde 経路 | `HealthStatus` の平坦化、傷と `Part`・`Stage`・傷跡・`Onset`・`Immunity` を保存対象に含める |
...
- [ ] `Wound` へ `Part`・`Stage` を足し、`woundEffect` で部位と深さから能力ペナルティを直接出す
- [ ] `WoundSystem` を足し、軽傷は代謝で自然治癒、深手だけ悪化・傷跡化、出血合計で HP を削る
...
- [ ] `Tend` を足し、治療量を負傷度の重い順に配る。浅い傷は複数部位にまたがり、深手は絞られる
