...
身体ペナルティと病気を、血という一本の生命ゲージと、部位ごとの傷で表す。部位ごとに耐久と傷を持たせ、どこを負ったかが並んで見える形にする。単独ターン制のサバイバルという ruins のジャンルに、この身体表現が噛み合う。機構は他ソフトから輸入せず、設計は ruins 既存の語彙で自己完結させる。
...
ただし**機能低下は引き続き既存6能力への `StatEffect` で表す**。部位は耐久と傷を持つが、部位ごとの機能値を別に算出する層は新設しない。部位の耐久は壊れるまでの丈夫さ、機能は6能力、生命は血、と役割を分ける。部位に耐久を持たせつつ機能は既存6能力へ集約する、という中庸に留める。
...
- 機能低下は既存6能力への `StatEffect` に写す。部位ごとの機能層を新設しない。部位の耐久は丈夫さに徹する。
...
- 病気は悪化と免疫の競走で決着する。主な効き目は機能低下で、致死性の病は臓器不全で死なせ、免疫が先に満ちれば治癒。出血は出血性の病だけが持つ。
...
- よく食べ休むほど速く回復する。代謝が血の再生と免疫の獲得を左右し、栄養と回復が繋がる。飢えれば治りが遅い。
...
classDiagram
    ...
    DiseaseSystem ..> HealthStatus : 致死病の終端で臓器不全死
...
sequenceDiagram
    ...
    loop 毎ターン
        ...
        alt Immunity が満ちた
            ...
            DS->>BP: 致死病は臓器不全で病死を確定
        end
    end
    Note over BL: 血が0の失血死、または機能不全の終端で死亡。OutcomeDied と DeathCause を確定し 260821223229 の終端へ
```
...
**死は2つの経路で訪れる**。血が尽きる失血死と、機能が尽きる臓器不全死。無理に一本化せず、外傷は血、環境と病は機能低下、と自然な役割で分ける。独立した即死フラグも、意識のような機能値の層も設けない。

- **失血死**: 外傷の出血が血を減らす。傷の bleed rate の合計、物理的な vital 部位破壊の大出血、出血性の病の出血。血が0で死ぬ。負傷死の主経路。
- **臓器喪失による即死**: body plan の vital 部位、人型なら頭か胴、の耐久が0になると臓器を失い、血を尽きさせる一撃の減算をその場で与える。即死フラグでなく大出血として血の経路へ流す。敵種で vital 部位が違い、蛇なら胴、ロボットなら中枢が的になる。
- **臓器不全死**: 低体温・高体温・飢餓・病気は機能を削り、それぞれの進行が満ちると臓器不全で直接死ぬ。血を経由しない。凍死・餓死・病死がこれで、`DeathCause` で区別する。多くの病は出血させず衰弱させて死なせるのが自然で、出血は出血性の一部の病だけが追加で持つ。
- **四肢の破損**: 腕・手・脚・足の耐久が0でも臓器を失わないので即死しない。破損として強い `StatEffect` ペナルティにし、傷からの出血は続く。

**代謝が回復を左右する**。血の再生速度と病気の免疫獲得速度は代謝で決まる。基礎は VIT、満腹度で上下し、一部の病気が下げる。よく食べ休めば速く治り、飢えれば遅い。血の回復と免疫の獲得を同じ代謝で束ねると、栄養と回復が自然に繋がる。
...
血ゲージは外傷を、機能低下を進める各条件は環境と病を表す。死因は失血と臓器不全の2系統に整理され、どちらも `DeathCause` つきの `OutcomeDied` へ合流する。
...
病気は悪化と免疫の競走で決める。悪化が振り切れば深刻化し、免疫が先に満ちれば治る。現状の `HealthCondition` は `Timer` が上下するだけでこの競走が無いので、免疫の軸をここだけ足す。
...
2. `Immunity` を免疫速度ぶん進める。速度は代謝 `Metabolism` を基礎に `SkillHealing` と治療が上乗せする。よく食べ休むほど速く治る
3. `Immunity` が100に満ちたら治癒として条件を除去する。同一ターンに両方が満ちたときも治癒を優先し、判定順の偶然で理不尽に死なせない
4. `Timer` が100で振り切れたら、致死性の病は臓器不全で直接 `OutcomeDied` の病死を確定する。血を経由しない。非致死の病は最大 severity で衰弱を続け、免疫が勝つのを待つ

病気の主な効き目は `Effects` の機能低下である。severity が上がるほど `StatEffect` のペナルティが強まり弱く遅くなる。死ぬのは致死性の病の終端だけで、大半の病は衰弱させて他の死因を招き寄せる。出血性の病は例外として `Bleeding` を追加で持つ。
...
各病気は raw のカタログで、機能低下の `Effects`、致死性か、出血性か、を持つ。致死性は終端で臓器不全死へ、出血性は `Bleeding` を追加する。風邪のように非致死の病は衰弱させるだけで、免疫が勝てば治る。
...
run を終える死は2系統ある。血が0になる失血死と、機能が尽きる臓器不全死。前者は傷の出血や物理的な臓器破壊、後者は低体温・飢餓・致死性の病の終端。`260821223229` の `RunOutcome` は `OutcomeDied` と `DeathCause` で受けるので `OutcomeKind` は増やさず、死因を失血・凍死・餓死・病死などの `DeathCause` で記録する。消耗が最終的にマクロループの終端へ合流する。
...
| `internal/systems/disease.go` | `DiseaseSystem` を新規追加し、悪化と免疫の競走を毎ターン進める。免疫獲得は代謝を基礎にし、致死性の病は終端で臓器不全死、出血性は `Bleeding` を足す |
| `internal/world/query/` | `Metabolism` を VIT・満腹度・条件から導出する派生関数を足す。血再生と免疫獲得が引く |
...
| `internal/systems/temperature.go` | 寒暑は機能低下の条件のまま。進行の終端で臓器不全死させ、低体温が Severe まで進んだら風邪の発症判定へ渡す |
| 飢餓経路 | 飢餓は機能低下のまま。進行の終端で餓死させる。満腹度は代謝にも効かせる |
...
- **失血を胴の耐久へ畳む**。血を独立させず、出血を胴の丈夫さの一部として扱う。却下。血を生命ゲージとして独立させたほうが、止血と安静という固有の対抗手段を出血に持たせられ、構造ダメージと分けて読める。
- **部位ごとの機能層を別に持つ**。耐久とは別に、意識・操作力・移動力などの連続的な機能値を部位から算出する層を新設する。却下。機能は6能力への `StatEffect` で表せ、器官の細分化と別層は必要が証明されてから。YAGNI。
...
- **病気を必ず出血させて死なせる**。全ての病死を血の減少で表す。却下。多くの病は衰弱させて死ぬのが自然で、出血は出血性の一部だけ。機能低下を主にし、致死性の病は臓器不全で直接死なせる。
- **回復速度を固定にする**。代謝を持たず血と免疫の回復を一定にする。却下。栄養状態が回復に効かないと食べる意味が薄れる。代謝で血再生と免疫獲得を束ね、飢えれば治りが遅い緊張を作る。

## コメント

死は失血と臓器不全の2系統に整理した。血 `Blood` を生命ゲージとして表示し、外傷の出血と物理的な臓器破壊が血を減らす。低体温・飢餓・致死性の病は機能低下の終端で臓器不全死させ、血を経由しない。血の上限・出血率・臓器喪失時の失血量・血の回復率・代謝の係数・傷の悪化速度・傷跡ペナルティ・病気の機能低下量と致死速度は着手後に実プレイで測る。傷跡は積もりすぎると死のスパイラルを生むので、1つあたりは軽くし、悪化速度は手当ての猶予を残す。傷跡を消す高度な治療は将来の余地として残す。敵種ごとの body plan の部位数と耐久配分も、戦闘の手触りを見てから調整する。`260821223229` と同じく、まだ触っていない手応えを机上で決め打たない。
...
- [ ] ④ `HealthCondition.Immunity` と `DiseaseSystem` を足し `InitializeSystems` へ登録する。致死性の病は臓器不全死、非致死は衰弱に留め、免疫獲得は代謝基礎にする
- [ ] ⑤ 感染創・風邪・疫病を足し、致死性・出血性の別と機能低下 `Effects` をカタログで持たせ、寒冷 Severe・腐敗食・放置した深手から発症させる
