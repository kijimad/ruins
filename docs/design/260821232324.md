...
classDiagram
    ...
    class DiseaseSystem
    class BleedSystem
    HealthStatus "1" o-- "部位数" BodyPartHealth : Parts
    ...
    BleedSystem ..> Wound : Bleeding を合算する
    BleedSystem ..> HealthStatus : Blood を減らし止血を判定する
```
...
sequenceDiagram
    ...
    AD->>BP: 部位 HP を減算
    AD->>BP: 武器由来の Wound を付与
    alt 頭か胴の HP が0
        AD->>BL: 臓器喪失の大出血
    end
    loop 毎ターン
        BS->>BP: 全部位の Wound.Bleeding を合算
        BS->>BL: 血を減らし止血済みの傷を閉じる
