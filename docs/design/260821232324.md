...
- 病気は潜伏期を持つ。感染から発症の自覚まで時間差があり、潜伏中は機能低下も表示も無く悪化と免疫だけが進む。
...
- 感染に気づかないまま進む怖さと予防の価値が生まれる。腐敗食や無手当ての傷が後から症状として返る。
...
- **放置で悪化**: 未治療の `WoundOpen` は `Severity` が上がる。深手閾値を超えると出血が増し、④の感染創の感染判定に掛かる。
...
```go
// HealthCondition に部位と発症閾値と免疫と致死性を足す
type HealthCondition struct {
	Type     ConditionType
	Part     BodyPart // 全身の病気は胴に置くなど、影響部位を持つ
	Severity float64
	Timer    float64 // 悪化度 0-100
	Onset    float64 // 発症閾値。Timer がこれ未満は潜伏期。機能低下も自覚も無い。病気のみ
...
病気は**潜伏期**を持つ。感染した直後は `Timer` が `Onset` 未満で、機能低下 `Effects` は効かず、不調リストにも現れない。背後で悪化 `Timer` と免疫 `Immunity` だけが進む。`Timer` が `Onset` を超えると**発症**し、機能低下が効き始めて初めて不調として自覚できる。感染から自覚まで時間差があり、腐敗食を食った時点では分からず、しばらくして症状が出て気づく。潜伏中は治療の対象にならないので、予防と早期の警戒が効く。潜伏の長さは病気ごとにカタログの `Onset` で定める。

病気の主な効き目は `Effects` の機能低下である。発症後、severity が上がるほどペナルティが強まる。死ぬのは致死性の病の終端だけで、大半は衰弱させて他の死因を招き寄せる。

### ④ 感染源と潜伏

感染の契機は新たな乱数源を単独で持たず、既存の曝露と選択へ紐づける。感染はここで起き、症状は③の潜伏を経て後から出る。
...
- **寒冷**: `Hypothermia` が Severe まで進んだら風邪の感染判定。
- **腐敗食**: `Perishable` の rotten 段階を食べたら疫病の感染判定。腐敗 doc `260815125506` の劣化段階を入力にする。
- **負傷**: 傷が放置で深手まで悪化したら感染創の感染判定。②の傷から連鎖する。

感染しても即座には分からない。潜伏期を経て発症し、そこで初めて不調として現れる。腐敗食を食う賭けや、傷を手当てせず進む判断が、後から症状として返る。
...
薬アイテムを消費して、出血を止め、傷を安定させ、HP を戻し、病気の悪化を鈍らせる。前方へ消費する資源の sink になる。全快の安全地帯は作らず、治療は前進の合間に行う。

治療の効き目は成否の二値でなく**治療品質**で決まる。品質は薬品のグレードと、治療スキルと操作機能から算出する。品質が回復速度とリスクの両方を左右する。

```go
// TreatQuality は治療品質 0-1。薬品グレードと SkillHealing と操作機能から決まる。
// 品質が回復速度とリスクを左右する。既存の ModHealingEffect も乗せる
func TreatQuality(world w.World, healer ecs.Entity, medicineGrade int) consts.Percent
```

- **薬品グレード**: 応急の布や間に合わせ < 標準の薬 < 高度な薬品。高いほど基礎品質が上がる。良い薬品は前方注文で得る scarcer な資源で、使う・売るの判断が薬にも及ぶ。
- **スキルと操作**: `SkillHealing` と身体機能の操作が品質を上げる。腕を負って操作が落ちると自分の手当ても下手になる。

治療品質の帰結:

| 品質 | 回復速度 | リスク |
|---|---|---|
| 高 | 止血が確実。傷が速く安定・治癒し、病気を強く鈍らせ免疫を速める | 深手でも傷跡になりにくく、感染しにくい |
| 低 | 止血が不完全。安定に失敗して `WoundOpen` のまま残りうる。鈍化が弱く回復も遅い | 手当てが裏目に出て感染創を招き、傷跡が残りやすい |

低品質の治療は賭けになる。粗末な手当てで急場をしのぐか、良い薬品を温存するか、の判断が生まれる。治療効果の算出は既存 `ModHealingEffect`・`SkillHealing` を再利用する。
...
| `internal/systems/disease.go` | `DiseaseSystem` を新規追加し、悪化と免疫の競走を進める。潜伏中は `Effects` と表示を抑え、`Timer` が `Onset` を超えたら発症させる。免疫速度に濾過を掛け、致死病は終端で死なせる |
| `internal/systems/helper.go` ほか初期化 | `WoundSystem`・`DiseaseSystem` を `InitializeSystems` へ登録する |
| `internal/systems/temperature.go` | 低体温 Severe で風邪の感染判定へ渡す。終端で死なせる |
| 飢餓・回復経路 | 満腹度を消化と循環・濾過の基礎に効かせる。HP 再生に循環を掛ける |
| `internal/activity/`（摂食・tend） | 腐敗食で疫病の感染判定。tend は治療品質に応じて止血・傷の安定・HP回復・病気鈍化を効かせ、低品質は感染や安定失敗のリスクを持つ |
| `internal/world/query/` | `TreatQuality` を薬品グレード・`SkillHealing`・操作機能から算出する |
| UI（ステータス） | 既存 HP バーへ、不調リストと身体機能の効率を並べる。潜伏中の病気は表示しない |
| serde 経路 | `HealthStatus` の平坦化、傷と `Part`・`Stage`・傷跡・`Immunity` を保存対象に含める |
| `raw.toml`・`internal/raw/` | 薬品のグレード、病気カタログの致死性・部位・発症閾値 `Onset`・`Effects` を定義する |
...
- **感染を即座に可視化する**。かかった瞬間に不調リストへ出す。却下。潜伏期の時間差が、予防の価値と、気づいたときには進んでいる緊張を生む。発症してから自覚できるほうがサバイバルの読みが深い。
- **治療を成否の二値にする**。手当ては成功か失敗かで、常に同じ効果。却下。薬品グレードと治療スキルと操作から治療品質を出し、品質で回復速度とリスクを連続的に変えるほうが、良い薬品を温存するか使うかの判断が生まれる。
...
- [ ] `HealthCondition` へ `Onset`・`Immunity`・`Lethal` を足し、`DiseaseSystem` を追加して `InitializeSystems` へ登録する
- [ ] 病気に潜伏期を入れる。`Timer` が `Onset` 未満は `Effects` と表示を抑え、超えたら発症させる
...
- [ ] `TreatQuality` を薬品グレード・`SkillHealing`・操作機能から算出する
- [ ] 治療アクションを足し、治療品質で回復速度とリスクを分ける。低品質は感染や安定失敗を招き、高品質は傷跡と感染を減らす
