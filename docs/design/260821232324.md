...
- 手当ては傷を即座に治さない。良い治療道具は、あとの治癒速度を上げ感染リスクを下げる。閉じるのは時間が担う。
...
classDiagram
    ...
    WoundSystem ..> Wound : Care と代謝で癒し、未処置深手は悪化
    WoundSystem ..> HP : 出血ぶん削る
    DiseaseSystem ..> HealthCondition : 免疫の競走
    Tend ..> Wound : 負傷度順に Care と止血を上げる
```
...
sequenceDiagram
    ...
    loop 毎ターン
        WS->>HP: 出血ぶん HP を削る
        WS->>HS: 傷を代謝と Care で癒し、未処置深手は悪化・膿む
    end
    Note over TD: 手当ては即座に治さず Care を上げる。閉じるのはその後の自然治癒。浅い傷は複数部位にまたがり深手は絞られる
```
...
// Wound は1つの傷。命中部位と、深さ・出血・手当ての質を持つ
type Wound struct {
	Type     WoundType
	Part     BodyPart
	Bleeding float64 // 出血の強さ。合算して毎ターン HP を削る。手当てと時間で下がる
	Severity float64 // 深さ 0-1。時間で下がって癒える。0で消える
	Care     float64 // 手当ての質 0-1。0が未処置。高いほど治癒が速く感染リスクが低い
	Deep     bool    // 深手だったか。癒えると傷跡として軽いペナルティを残す
}
...
傷の種類は既存の武器スキルから導く。命中部位は部位サイズの重みで抽選する。傷は毎ターン `WoundSystem` が進める。**傷は時間で `Severity` が下がって癒える。治癒速度は代謝と手当ての質 `Care` で決まる**。手当ては傷を即座に治さず、あとの治癒を速め感染リスクを下げるだけで、実際に閉じるのは時間が経ってから。良い治療道具ほど `Care` が高く、治りが速く感染しにくい。未処置で `Care` が低い深手は治りが遅く、膿んで感染創の感染判定に掛かりやすい。軽い掠り傷は手当てせずとも代謝で自然に癒え、感染も稀なので、手当てが要るのは深手に絞られる。出血は手当てと時間で下がり、止血が急がれるのは止まる前に HP を脅かす強い出血だけ。深手だった傷は癒えても傷跡として軽いペナルティを残す。良い手当ては傷跡の残りも減らす。演出はログと敵ステータスに出す。命中時に `左腕を切り裂いた！ 出血` とログし、ステータス欄へ HP と傷と病気を並べる。
...
```go
// Tend は1回の手当て。治療量 budget を負傷度の重い順に配り、各傷の Care と止血を上げる。
// 傷を即座には治さない。あとの治癒速度を上げ感染リスクを下げるだけ
func Tend(hs *HealthStatus, budget float64)
```

- 治療量 `budget` は薬品のグレードと量、`SkillHealing` から決まる。良い道具ほど大きい。
- 出血する傷を最優先に、次いで深手から順に手当てする。手当ては傷の `Care` を上げて治癒を速め感染リスクを下げ、出血を止める。1傷に要る量は `Severity` に比例する。
- 浅い傷は少量で済むので、1回の手当てが複数部位にまたがる。深手は量を食うので、`budget` が尽きれば残りは未処置のまま次の手当てへ持ち越す。
- 手当ては即座に傷を閉じない。閉じるのはその後の自然治癒で、手当ての質がその速度と安全を決める。
- `budget` が余れば発症中の病気の悪化を鈍らせる。

**手当ては戦闘のたびの routine ではない**。多くの掠り傷は代謝で勝手に癒えるので、手当てが要るのは、放置すれば悪化する深手、自然に止まらない強い出血、発症した病気、という危険な場合だけ。危機の局面でだけ薬を切る判断になる。

プレイヤーは「手当て」を選ぶだけで、どこから手当てするかは負傷度で自動に決まる。深手が多いほど1回で足りず薬が要る。良い薬ほど治りが速く感染しにくい。傷ついた→薬で手当て→時間で癒える、のループが、良い薬を使うか温存するかの判断とともに回る。全快の安全地帯は作らず、前進の合間に手当てする。治療効果は既存 `ModHealingEffect`・`SkillHealing` を再利用する。
...
| `internal/systems/wound.go` | `WoundSystem` を追加し、傷を `Care` と代謝で毎ターン癒し、未処置の深手は膿んで悪化・感染させ、深手の治癒後に傷跡を残す。出血合計で HP を削る |
...
| `internal/activity/`（新規 tend アクション） | `Tend` を追加し、薬を消費して治療量を負傷度順に配り、各傷の `Care` と止血を上げる。傷は即座に治さず、あとの治癒速度と感染リスクを変える |
...
| serde 経路 | `HealthStatus` の平坦化、傷と `Part`・`Care`・`Deep`・傷跡・`Onset`・`Immunity` を保存対象に含める |
...
- **手当てで傷が即座に治る**。薬を使うと傷が消える。却下。非現実的で、傷の重みが消える。手当てはあとの治癒速度を上げ感染リスクを下げるだけにし、閉じるのは時間が担う。良い道具ほど治りが速く安全になる、という差にする。
...
- [ ] `Wound` へ `Part`・`Care`・`Deep` を足し、`woundEffect` で部位と深さから能力ペナルティを直接出す
- [ ] `WoundSystem` を足し、傷を `Care` と代謝で癒し、未処置の深手は膿んで悪化・感染、深手治癒後に傷跡、出血で HP を削る
...
- [ ] `Tend` を足し、治療量を負傷度順に配って各傷の `Care` と止血を上げる。即座に治さず治癒速度と感染リスクを変える
