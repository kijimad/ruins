...
本ドキュメントは広範なビジョン設計であり、多くは擬似コードによる構想である。以下はコードベースで実在を確認できた成果物に基づく棚卸しである。

### 環境温度・体温

- [x] 環境気温の算出。基本気温＋時間帯修正＋タイル修正の合算。`internal/systems/temperature.go` の `CalculateEnvTemperature`、`internal/dungeon/definition.go` の `BaseTemperature`
- [x] タイル単位の気温修正。屋内/屋外の遮蔽、水辺、植生。`internal/components/tile_temperature.go` の `ShelterType`／`WaterType`／`FoliageType`
- [x] 時間帯サイクルと気温変動。夜明けから深夜まで。`internal/components/game_time.go` の `TimeOfDay`、`GetTemperatureModifier`
- [x] 低体温症・高体温症の進行と能力低下。`internal/components/health_status.go` の `ConditionHypothermia`／`ConditionHyperthermia`、`internal/systems/temperature.go` の `updateTemperatureConditions`
- [x] 寒波前線の極低温ゾーンによる極寒脅威。`internal/systems/temperature.go` の `frostZoneModifier`、`SeamlessBand.Front.InColdZone`
- [~] 明示的な体温値と段階。`BodyTemperature`／`TempCold`／`TempCritical`／`TempDeath`。状態タイマー方式で代替しており、数値モデルは未実装
- [~] 凍傷。露出部位への `Frostbite` 発生。該当コードなし
- [~] 火・暖房システム。薪・マッチ・ライターでの着火、火による加温と乾燥。`StartFire`／`Fire` 相当のコードなし
- [~] 濡れ・防水・乾燥のメカニクス。`IsWet`／`Waterproof` 相当のコードなし
- [~] 風冷と防風。`WindChill`／`Windproof` 相当のコードなし
- [~] 扉の開閉による部屋間の温度混合。タイル単位のため部屋モデル自体が未実装

### ニーズ

- [x] 空腹ニーズと段階別ペナルティ。`internal/components/hunger.go` の `Hunger`／`HungerLevel`
- [~] 渇き・休息・快適さ・士気・痛み・美しさのニーズ。`Thirst`／`Morale`／`Pain`／`Beauty` 等のニーズプールは未実装

### 装備

- [x] 装備による断熱。耐寒・耐暑の合算。`internal/components/components.go` の `Wearable.InsulationCold`／`InsulationHeat`、`internal/systems/temperature.go` の `CalculateEquippedInsulation`
- [x] 重量と所持重量容量。`internal/components/components.go` の `Weight`／`WeightCapacity`
- [~] 防寒と戦闘のトレードオフ。回避・攻撃ペナルティ、柔軟性。`Wearable` は `Defense` のみで戦闘ペナルティ項目なし
- [~] レイヤーシステム。重ね着の合算と3層ボーナス。該当コードなし
- [~] 戦闘による装備消耗・破損・応急修理。`Durability`／`CombatWearRate` 相当のコードなし
- [~] 装備品質システム。粗悪から伝説までの性能倍率、深度連動生成。装備の `Quality` は未実装

### 健康・思考

- [x] 部位別の健康状態の枠組み。頭・胴・腕・脚・全身。`internal/components/enum.go` の `BodyPart`、`internal/components/health_status.go` の `BodyPartHealth`
- [x] 耐寒・耐暑スキルによる体温進行の緩和。`internal/components/modifiers.go` の `ColdProgress`／`HeatProgress`、`SkillColdResist`
- [~] 部位別の負傷・出血・治療。切り傷・打撲・包帯。`Injury`／出血相当のコードなし
- [~] 思考システムと気分への影響。`Thought` 相当のコードなし

### イベント・敵・ゲームループ

- [~] AIストーリーテラー。`EventManager`・脅威レベル・イベント抽選。該当コードなし
- [~] 天候システム。晴れ・曇り・吹雪。名前付き天候は未実装、寒波前線ゾーンのみ実装済み
- [~] 極寒特化の敵。狼の群れ・熊・野生犬・猛禽類。汎用の `Boss` マーカーはあるが専用敵は未実装
- [~] 雪崩・氷の床などの環境的脅威
- [~] 生存ゲームループ。拠点準備・帰還判断・死亡サマリー
- [~] 社交NPC・エンドコンテンツ。山を下り切る到達目標
