...
- **採らなかった自然物**: `plant` はスプライトが観葉植物、鉢植え、で屋外に合わない。`moving_stone` は紫水晶、
  `stone_pillar` は加工された石柱で、いずれも自然の岩に見えない。自然な岩・草アイテムのスプライトが起きたら足す。
...
- **スプライトは実物を確認して選ぶ**。名前が自然物でも中身が屋外向きとは限らない。`plant` は観葉植物、
  `moving_stone` は紫水晶で、原野では浮いた。VRT と個別スプライトで見た目を確かめ、樹木に絞った。自然な岩や
  草アイテムのスプライトが起きたらカタログへ足す。
