...
裸の dirt だけだと殺風景なので、地面へ草の房を疎に撒いて野原の下地を作る。dirt を歩行可能なオートタイル床
`grass_olive`・`grass_green` へ置換する。road.go の `replaceTile` を流用し、置換後のオートタイル添字は既存の
`RecalcAutotileInXRange` が実状態から揃える。草地は `BlockPass` を持たないので通行を塞がず、位相格子や経路
マスクの対象にしない。凍てついた世界観に合わせ大半をくすんだオリーブにし、みずみずしい緑をまばらに混ぜて単調さを
崩す。prop は草の上にも立てるよう、草を先に敷いてから撒く。

草タイルのオートタイルは「四方を同種で囲まれた面」が暗いフィルになり、大きく塗りつぶすと沼のように見える。
そこで密度を低く保って房どうしを離し、常に `dirt` の上の草むらとして描かせる。並びは `ScatterArea` の hash
ジッタが担うので格子状にならず、孤立した草むらが不規則に散る。密度を上げると小さな草地の塊が繋がり、面ではなく
まだらのまま茂りを増せる。
...
const (
	zoneRoadside outdoorZone = "roadside" // 道・集落・市街に近い。踏み分けられた原で草地は疎、雑草と岩が主
	zoneWild     outdoorZone = "wild"     // 奥地。草地が濃く、灌木・木立・岩が茂る
)
```
...
- 寒さは温度場、屋外の物は prop、と役割分担を保つ。将来、地形写像テーブルで温度からカタログを差し替え「寒い
  チャンクは枯れ版」を純関数で表せる余地を残す。地表タイルの拡張には当面踏み込まない。ただし草地は既存の
  歩行可能オートタイル床の流用なので、新タイル追加でなく地表の色変えに留まる。
- **原野の主役は自然物にする**。当初は放置車両・ドラム缶など人工物を主に据えたが、原野一面に人工物を撒くと
  不自然で「話にならない」ほど景色が壊れた。CDDA の野原・森が草地と植生を下地に、人工物は稀な map_extra で
  局所的に置くのに倣い、v1 は草の房＋樹木を主役にし、人工物の散布は見送る。
- **スプライトは実物を確認して選ぶ**。名前が自然物でも中身が屋外向きとは限らない。`plant` は観葉植物、
  `moving_stone` は紫水晶で、原野では浮いた。VRT と個別スプライトで見た目を確かめ、樹木に絞った。岩は自然な
  スプライトが無かったのでダミーの仮スプライト `dummy_rock` を起こして登録した。正規の岩・草スプライトが起きたら差し替える。
...
- [x] `scatterCatalog`/`scatterEntry` の屋外カタログを実装する。地面へ草の房を撒き、樹木、低木・大木、を重み付きで撒く。観葉植物や水晶など屋外に合わないスプライトと人工物は v1 では置かない。置かない選択は null 重みで表す。個数は `count = round(area * Density)` で密度確率から導く
