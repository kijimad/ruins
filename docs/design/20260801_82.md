...
prop**、`dummy_grass`・`dummy_weeds`、として地面に重ねる。当初は `grass_olive`・`grass_green` のタイルへ
...
- 大半をみずみずしい草にし、乾いた雑草 `dummy_weeds` をまばらに混ぜて単調さを崩す。
...
- **草**: `dummy_grass`・`dummy_weeds`。透明背景の prop で地面に重なる。密に撒いて野原の質感を出す。歩行可能。
- **樹木**: `tree_a`・`tree_b`、低木、と `big_tree`、大木。緑葉の樹木スプライトで原野に馴染む。`big_tree` だけ
  `Big` で位相格子と経路マスクの対象。低木は疎に撒き、通行を塞いでも希釈と外周の空きで連結を保つ。
- **岩**: `dummy_rock`。自然な岩スプライトが無かったのでダミーの仮スプライトを起こして登録した。乗り越えられる
...
- **スプライトは実物を確認して選ぶ**。名前が自然物でも中身が屋外向きとは限らない。`plant` は観葉植物、
  ...
  スプライトが無かったのでダミーの仮スプライト `dummy_grass`・`dummy_weeds`・`dummy_rock` を起こして登録した。
  正規のスプライトが起きたら差し替える。
...
- [x] VRT で wasteland の道沿いの散布を目視し、密度とカテゴリ比を確認した。道上に大物が乗らず横断レーンが空くことも VRT で確認
