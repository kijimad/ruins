...
原野一面に prop を撒くと不自然なので、まず地面そのものを草地に変えて野原の下地を作る。dirt を歩行可能な
オートタイル床 `grass_olive`・`grass_green` へ確率的に置換する。road.go の `replaceTile` を流用し、置換後の
オートタイル添字は既存の `RecalcAutotileInXRange` が実状態から揃える。草地は `BlockPass` を持たないので通行を
塞がず、位相格子や経路マスクの対象にしない。凍てついた世界観に合わせ大半をくすんだオリーブにし、時折みずみずしい
緑を混ぜて単調さを崩す。prop は草地の上にも立てるよう、草地を先に敷いてから撒く。
...
- `zoneRoadside` は踏み分けられた原なので草地を疎に、雑草と岩を主に灌木は少なめ。`zoneWild` は草地を濃く、
  灌木・雑草・岩を茂らせ時折木立を立てる。
- 個数は密度確率で決め、`count = round(area * Density)` を `ScatterArea` へ渡す。草地と prop で別々の密度係数を持つ。
  `area` はチャンクのタイル数で、interior の `selectTiles` が `Rect.W*Rect.H` を candidates にするのと同じ単位。

### prop カタログ

実スプライトのある自然物を使う。原野の主役は自然物にし、人工物を一面に撒くと不自然になるので v1 では置かない。

- **自然物**: `plant`、雑草、と `moving_stone`、石、は `BlockPass` を持たず歩行可能なので密に撒ける。`tree_a`・
  `tree_b`、灌木、と `big_tree`、木、は通行を塞ぐので疎にする。`big_tree` だけ `Big` で位相格子と経路マスクの対象。
...
- 束の例: 木立は `big_tree` を anchor に `tree_a`・`tree_b`・`plant` を随伴し、大木の周りに下生えが寄り添う小景を作る。
...
- **原野の主役は自然物にする**。当初は放置車両・ドラム缶など人工物を主に据えたが、原野一面に人工物を撒くと
  ...
  局所的に置くのに倣い、v1 は草地＋灌木・雑草・石を主役にし、人工物の散布は見送る。
- 仮画像 prop は避け実スプライトを優先する。放置車両・道端の残骸を「道沿いにまれに」出す人工物層は、稀度制御と
  占有共有が要るので後続の doc で足す。
...
- [x] `scatterCatalog`/`scatterEntry` の屋外カタログを実装する。地面を草地へ置換し、自然物、灌木・雑草・石・木、を重み付きで撒く。人工物は原野一面では不自然なので v1 は見送る。置かない選択は null 重みで表す。個数は `count = round(area * Density)` で密度確率から導く
