...
- **場所寄せ `placeFactor(northDepth, next)`**: `1 + severity(next) * northDepth * k`。severity は明示表で
  持ち、晴れ0・曇り0・雨0・雪1・吹雪2・寒波2。並び順とは一致しない。プレイヤーの奥行き
  `NorthDepthChunks` が大きいほど厳しい天候の重みだけが伸び、北へ進むほど吹雪・寒波が主役になる。雨は
  severity0 で奥行きに反応せず、雪・吹雪が伸びる分だけ相対比が縮む。深部は寒すぎて雨が雪になる現実に沿う。
  緯度が寒くなるのと同じ空間軸で、進行=北進のこのゲームでは場所が進行度を兼ねる。

- **スペル長 `RollSpellTurns`**: 天候種ごとの基準ターン数を持ち、季節・場所で伸縮させて乱数で散らす。
  `turnsPerDay=1000` が1日なので、例えば晴れ 1000〜3000、曇り 800〜2000、雨 500〜1500、吹雪 200〜800 の
  ように、晴れは長く吹雪は短い。雨は通り雨から長雨まで中くらいで散らす。冬・奥地では厳しい天候の基準を
  伸ばし、荒天が居座る。現実のスペル長は指数分布に近いので下限+乱数で裾を作る。値は暫定。
...
- **視界**: `VisionSystem` は視程半径を定数 `consts.VisionRadiusTiles` から作る（`vision.go:82`）。ここを
  `VisionRadiusTiles + Effect().VisionDelta` の実効タイル数へ変え、下限で床を打つ。吹雪でも0にはせず
  最低限は見える。敵の索敵半径 `enemy.go`・`aiinput` も同じ定数を使うが、天候で敵の索敵まで縮めるかは
  スコープ判断で、今回はプレイヤーの視界描画だけに効かせる。
