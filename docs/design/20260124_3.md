...
**タスク**:
- [x] `Stackable` コンポーネントの使用箇所をリストアップ
- [x] `HasComponent(world.Components.Stackable)` の出現箇所を特定
- [x] Rawデータ（TOML）の `stackable` フラグ使用状況を確認
- [x] テストケースの洗い出し

**対象ファイル**:
- `internal/components/components.go`
- `internal/worldhelper/spawn.go`
- `internal/worldhelper/inventory.go`
- `internal/worldhelper/stackable.go`
- `internal/worldhelper/weight.go`
- `internal/worldhelper/shop.go`
- `internal/worldhelper/craft.go`
- `internal/raw/raw.go`
...
**タスク**:
- [x] `Item` コンポーネントに `Count` フィールドを追加
```go
type Item struct {
    Count int  // デフォルト: 1
}
```
- [x] `Stackable` コンポーネントを空の構造体に変更（マーカーとして機能）
```go
type Stackable struct {
    // 空でOK（存在自体がスタック可能性を示す）
}
```
- [x] `NewItemSpec` を修正し、`Item.Count` を設定するように変更
```go
...
**タスク**:
- [x] `ChangeItemCount` を実装
- [x] `GetItemCount` を修正
- [x] `ConsumeItem` を簡略化
- [x] `SpawnItem` を簡略化（ループ削除）
- [x] `CalculateCurrentCarryingWeight` から分岐を削除
- [x] `AddStackableCount` を `AddItemCount` にリネーム
- [x] すべての呼び出し箇所を更新
...
**タスク**:
- [x] `spawn_test.go`: Item.Count を使用するテストに更新
- [x] `inventory_test.go`: Item.Count を使用するテストに更新
- [x] `weight_test.go`: Item.Count を使用するテストに更新
- [x] `shop_test.go`: Item.Count を使用するテストに更新
- [x] `craft_test.go`: Item.Count を使用するテストに更新
- [x] Stackableコンポーネントはマーカーとして残るため、存在チェックのテストは維持
- [x] すべてのテストが通ることを確認
...
**タスク**:
- [x] `ChangeStackableCount` を削除（`ChangeItemCount` に置き換え済み）
- [x] `AddStackableCount` を削除（`ChangeItemCount` に置き換え済み）
- [x] `Stackable.Count` フィールド参照箇所がないことを確認
- [x] 不要になったヘルパー関数を削除
- [x] コンパイルエラーがないことを確認
- [x] すべてのテストが通ることを確認
...
**タスク**:
- [x] `docs/design.md` を更新
- [x] アイテムシステムの設計ドキュメントを更新
- [x] コード内コメントを更新
- [x] CHANGELOG.md に破壊的変更を記載
...
コードベースを棚卸しした結果を記録する。本ドキュメントの中核提案である「全アイテム共通の `Item` コンポーネントへ `Count` を移し、`Stackable` を空のマーカーにする」方式は未採用である。実際の実装は `Count` を `Stackable` に残したまま進んでおり、`Item` という名前のコンポーネントは存在しない。そのため提案どおりの成果物の多くは未実装となる。判定は主に `internal/` のコードで裏取りした。

### データ構造の変更

- [ ] `Item` コンポーネントへ `Count` フィールドを追加。`Item` コンポーネント自体が存在せず、個数は従来どおり `Stackable.Count` に残る。`internal/components/components.go:136-138`
- [ ] `Stackable` を空のマーカー構造体に変更。依然として `Count int` を保持している。`internal/components/components.go:136-138`
- [ ] `NewItemSpec` で `Item.Count` を設定。`internal/raw/raw.go:312` は空の `Stackable{}` を付与し、個数は `internal/world/lifecycle/spawn.go:285` で `Stackable.Count` へ設定している

### 関数の統一

- [x] `ChangeItemCount` を実装。`internal/world/lifecycle/inventory.go:15` に存在する。ただし内部は `Stackable.Count` を増減する実装で、引数は `world, entity, delta` であり、提案の `Item.Count` 版とは異なる
- [ ] `GetItemCount` を統一。同名関数はなく、`GetEntityCount` が `Stackable.Has` で分岐し `Stackable.Count` を返す。`internal/world/query/inventory.go:55`
- [ ] `ConsumeItem` を簡略化。`ConsumeItem` 関数は存在しない。消費は各所で `ChangeItemCount(..., -1)` を直接呼んでいる。`internal/activity/use_item.go:116` など
- [x] `SpawnItem` の非Stackable複数生成ループを削除。`internal/world/lifecycle/spawn.go:274-277` は非Stackableで `count > 1` をエラーにし、複製ループは残っていない
- [ ] `CalculateCurrentCarryingWeight` から分岐を削除。同名関数はなく、重量計算は `GetEntityWeight` から `GetEntityCount` を経由して `Stackable.Has` 分岐が残る。`internal/world/query/location.go:30`、`internal/world/query/weight.go`
- [ ] `AddStackableCount` を `AddItemCount` にリネーム。どちらの関数もコード上に存在しない
- [ ] すべての呼び出し箇所を `Item.Count` へ更新。`Item.Count` 設計が未導入のため対応していない

### 旧関数の削除とクリーンアップ

- [ ] `ChangeStackableCount` を削除。`internal/world/lifecycle/inventory.go:48` に現存し、`shop.go`、`craft.go`、`states/factories.go` から呼ばれている
- [ ] `Stackable.Count` 参照箇所の除去。生成・個数取得・重量計算など多数の箇所で現役の参照が残る

### テストの更新

- [ ] テストを `Item.Count` 対応へ更新。`internal/world/lifecycle/inventory_test.go`、`stackable_test.go` などは `gc.Stackable{Count: N}` を用いており `Item.Count` へは移行していない

### ドキュメント更新・事前調査

- [ ] `docs/design.md` 等のドキュメント更新。`internal/` のコードからは確認できない
- [ ] Phase 1 の影響範囲調査。調査タスクであり、コード上の成果物として確認できない
