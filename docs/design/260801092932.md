...
- **決定的な純関数**。チャンク seed `ChunkSeed2D(runSeed, cx, cy)` から一括生成し、再訪で一致し serde 安全であること。
  ...
  可変状態を使わない。
- **doc 260725201431.md の密度場を流用**。Room 依存を解いて屋外の開領域へ density 配置を効かせる。車輪の再発明をしない。
...
doc 260725201431.md の `selectTiles(room, p, occupied, seed, count)` が Room から読むのは `Rect` と `Doorways` だけである。屋外は
戸口を持たない開領域なので、密度場の芯を Room 非依存の純関数として切り出し、interior と overworld の双方から呼ぶ。
これは doc 260731225939.md、doc 260725201431.md のバックログ、の「地点セレクタ API への統一リファクタ」を屋外流用の要求で前倒しする。
...
通行を塞ぐことがない。「1タイル1物」は doc 260725201431.md の密度場が元から持つ性質で、別機構は要らない。撒く物は zone ごとの
...
doc 260725201431.md のパリティ格子、`(x+y)%2==0`、を一般の P 位相へ広げる。候補タイルを `(x + k*y) mod P` の位相へ分け、`Big` の
...
### 草は prop で撒く —— タイルのオートタイルを避ける

裸の dirt だけだと殺風景なので、地面へ草の房を密に撒いて野原の質感を出す。草は**タイルでなく透明背景の
prop**、`grass`・`weeds`、として地面に重ねる。当初は `grass_olive`・`grass_green` のタイルへ
置換したが、これらは autotile 床で、同種で囲まれた面は中央が暗いフィルになり大きく塗ると沼のように見え、異種の
境界は明るい縁取りになって自然に繋がらない。autotile は正しく適用されており、`RecalcAutotileInXRange` が4近傍
から添字を揃えるが、この tileset の草・砂タイルは「縁の装飾」用で面の塗りに向かない。prop なら autotile を
経由せず、草のスプライトが土の上へそのまま重なり、土から草が生える自然な絵になる。CDDA が地形の質感を境界線でなく
重ね描きで表すのと同じ発想である。

- 草 prop は歩行可能、`BlockPass=false`、なので通行を塞がず、位相格子や経路マスクの対象にしない。
- 大半をみずみずしい草にし、乾いた雑草 `weeds` をまばらに混ぜて単調さを崩す。
- 並びは `ScatterArea` の hash ジッタが担うので格子状にならず、不規則に散る。密度で茂りを調整する。
- 樹木が草と重ならないよう、草を先に撒いて占有を記録し、樹木はそれを避ける。

### ゾーン重み —— 人の手の残り具合で撒き分ける

wasteland チャンクの「集落・市街地への近接度」を2ゾーンに分け、カタログと密度を切り替える。道は集落間を結ぶので、
集落の近傍が道沿いの帯を兼ねる。中間帯は必要が出てから足す、YAGNI。

```go
// outdoorZone は wasteland チャンクの人の手の残り具合。近傍の集落・市街地までの距離で決まる。
// 実体は文字列にし、%v やログで種別名が読める。iota の連番は割に合わないので使わない。
type outdoorZone string

const (
	zoneRoadside outdoorZone = "roadside" // 道・集落・市街に近い。踏み分けられた原で草も低木も疎
	zoneWild     outdoorZone = "wild"     // 奥地。草が濃く、低木・木立・岩が茂る
)
```

- `zoneRoadside` は踏み分けられた原なので草も低木も疎。`zoneWild` は草の房を濃く、低木を茂らせ時折木立を立てる。
- 個数は密度確率で決め、`count = round(area * Density)` を `ScatterArea` へ渡す。草と prop で別々の密度係数を持つ。
  `area` はチャンクのタイル数で、interior の `selectTiles` が `Rect.W*Rect.H` を candidates にするのと同じ単位。

### prop カタログ

実スプライトのある自然物だけを使う。原野の主役は自然物にし、人工物を一面に撒くと不自然になるので v1 では置かない。
スプライトを実際に確認して、屋外の原野に馴染む物だけを選ぶ。

- **草**: `grass`・`weeds`。透明背景の prop で地面に重なる。密に撒いて野原の質感を出す。歩行可能。
- **樹木**: `tree_a`・`tree_b`、低木、と `big_tree`、大木。緑葉の樹木スプライトで原野に馴染む。`big_tree` だけ
  `Big` で位相格子と経路マスクの対象。低木は疎に撒き、通行を塞いでも希釈と外周の空きで連結を保つ。
- **岩**: `rock`。自然な岩スプライトが無かったのでダミーの仮スプライトを起こして登録した。乗り越えられる
  よう `blockPass=false` にし `passCost` で歩きにくさだけ表す。連結性を壊さない。正規の岩スプライトが起きたら差し替える。
- **採らなかった既存スプライト**: `plant` はスプライトが観葉植物、鉢植え、で屋外に合わない。`moving_stone` は
  紫水晶、`stone_pillar` は加工された石柱で、いずれも自然の岩に見えないので使わない。
- **人工物は見送り**: `barrel`・`crate`・`forklift`・`vending_machine` などの人工物を原野一面に撒くと不自然なため
  v1 では散布しない。放置車両や道端の残骸を「道沿いにまれに」出す層は、占有共有と稀度制御が要るので後続にする。
- 束の例: 木立は `big_tree` を anchor に `tree_a`・`tree_b` を随伴し、大木の周りに低木が寄り添う小景を作る。
- 束の境界と失敗の責務は散布側、すなわち `scatterFeature` が持つ。`ScatterArea` は anchor タイルを選ぶだけで、
  `Satellites` の展開はしない。衛星は自然地面かつ非占有に収まらなければ落とす。anchor だけ置いて衛星を諦める、
  doc 260725201431.md の `Satellites` と同じ「尽きたら諦める」方針にする。
...
| 森の component 連鎖、`one_in`→pick、break | 不採用。doc 260725201431.md の重み付き密度選択に等価で冗長。null 重みで代替する |
...
| 実測重み、35000:6:2 等 | 初期値に借用。doc 260725201431.md の実測比率初期値化と同じ扱い |

**ruins 固有で CDDA と違う点**。決定性を hash 純関数へ徹底し、再計算で一致させる、CDDA はセーブ保存。配置の芯は
doc 260725201431.md の密度場、ruins 自前。通行性を位相格子の Big 希釈と seed 経路マスクの二段で構造保証する、CDDA は確率的希釈と
...
| `internal/mapplanner/interior/scatter_golden_test.go`(新規) | 密度場の芯 `ScatterArea` の点選定分布を画像 golden 化。密度別の広がり・accept 絞り込み・外周除外を目視する |
| `internal/overworld/scatter_golden_test.go`(新規) | 散布を含む wasteland の生成結果を ECS から直接 PNG 化する生成レベル golden。State ごとの states golden とは分ける |
| `docs/design/260731225939.md` | 「地点セレクタ API 統一リファクタ」を本 doc の `ScatterArea` 切り出しで一部進めたと追記で接続 |
...
- **CDDA の component 連鎖を独立機構として持つ** —— `one_in(chance)` の sequence で1タイル1物を保証する仕組みは、
  doc 260725201431.md の密度場が「選んだタイルへ重み付きで1個」で既に満たす。null 重みで置かない選択も表せるので、別機構は冗長。
  自前の密度場に畳む。
...
- **`RuinsDebris` の距離ベース確率散布を流用** —— 廃墟プランナー専用でタイル置換のみ。エンティティ化・チャンク境界・
  決定的 seed が未対応。密度場のほうが doc 260725201431.md と一貫し、占有回避・束・ゾーン重みを表せる。
...
- 本 doc は `20260725_70`、施設内装、と対をなす屋外の器であり、doc 260725201431.md の density placement の初の外部流用でもある。
...
- **草はタイルでなく prop で撒く**。地面のムラをタイルで作ろうとしたが、autotile は同種の面を暗い中央フィルに、
  異種の境界を明るい縁取りにするため、単色崩しにも草地にも使えなかった。autotile 自体は正しく適用されている、
  `RecalcAutotileInXRange` が4近傍から添字を揃える、が、この tileset の草・砂タイルは縁の装飾用で面の塗りに
  向かない。透明背景の prop を地面へ重ねる方式に切り替え、縁も暗いフィルも出さずに土から草が生える絵にした。
- **原野の主役は自然物にする**。当初は放置車両・ドラム缶など人工物を主に据えたが、原野一面に人工物を撒くと
  不自然で「話にならない」ほど景色が壊れた。CDDA の野原・森が草と植生を下地に、人工物は稀な map_extra で
  局所的に置くのに倣い、v1 は草＋樹木を主役にし、人工物の散布は見送る。
- **スプライトは実物を確認して選ぶ**。名前が自然物でも中身が屋外向きとは限らない。`plant` は観葉植物、
  `moving_stone` は紫水晶で、原野では浮いた。VRT と個別スプライトで見た目を確かめ、樹木に絞った。草と岩は自然な
  スプライトが無かったのでダミーの仮スプライト `grass`・`weeds`・`rock` を起こして登録した。
  正規のスプライトが起きたら差し替える。
- 放置車両・道端の残骸を「道沿いにまれに」出す人工物層は、稀度制御と占有共有が要るので後続の doc で足す。
...
- [x] `scatterCatalog`/`scatterEntry` の屋外カタログを実装する。草・雑草を透明 prop で地面へ密に重ね、樹木、低木・大木、と岩を重み付きで撒く。草タイルは autotile の縁・暗いフィルが出るので prop 方式にした。観葉植物や水晶など屋外に合わないスプライトと人工物は v1 では置かない。個数は `count = round(area * Density)` で密度確率から導く
- [x] 位相格子を実装し、`Big` の大物を1位相へ寄せて実効密度を 1/P に希釈する。doc 260725201431.md のパリティ格子の一般化
...
- [x] VRT で wasteland の道沿いの散布を目視し、密度とカテゴリ比を確認した。道上に大物が乗らず横断レーンが空くことも VRT で確認。VRT は overworld の生成結果を直接描く生成レベル golden にし、State ごとの states golden から分けた
- [x] doc 260731225939.md の placement セレクタ統一の項へ本 doc の `ScatterArea` 切り出しを追記で接続する
