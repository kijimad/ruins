...
classDiagram
    ...
    FrontlineX ..> GameTime : 時刻を読む
    FrontlineX ..> SeamlessBand : 前進速度を読む
    FrostRenderSystem ..> FrontlineX : 描画位置を引く
    ColdPursuitSystem ..> FrontlineX : 追いつき判定に使う
    ColdPursuitSystem ..> RunOutcome : 敗北を確定する
...
`ColdPursuitSystem` が確定するのは追いつきの `OutcomeConsumed` だけで、生還判定と力尽きはそれぞれの担当が `RunOutcome` へ書き込む。

```mermaid
sequenceDiagram
    participant GT as GameTime
    participant CP as ColdPursuitSystem
    participant FX as FrontlineX
    participant SV as 生還判定
    participant BD as 身体系system
    participant RO as RunOutcome
    participant ST as 終端State

    loop 毎ターン
        CP->>FX: 現在の前線Xを要求
        FX->>GT: TotalTurns を読む
        FX-->>CP: 前線X
        CP->>CP: プレイヤー絶対Xと比較
        alt 前線が追いついた
            CP->>RO: OutcomeConsumed を確定
        else 終端条件を満たした
            SV->>RO: OutcomeEscaped を確定
        else 力尽きた
            BD->>RO: OutcomeDied と DeathCause を確定
        end
    end
