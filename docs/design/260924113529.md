...
| レシピ(Group/Stuff) | `content_catalog.go` 17関数 (Go) | raw.toml `interiorContents` |
| 部屋→content写像 | content_catalog.go:309 (Go) | raw.toml(interiorContents に room を含める) |
...
model ContentStuff {
  ...
  ref: EntityID;
  /** pick_one / pick_n の抽選重み。0 は 1 とみなす */
  weight: EntryWeight;
  /** 0..100。pick_each でこの Stuff を置く確率。0 以下は常置 */
  chance: int32;
  /** 置く個数のダイス表記 */
  amount: Dice;
  /** どこへ置くか。空なら archetype の既定へ落ちる */
  placement?: Placement;
}
...
`Raws` に `interiorContents: InteriorContent[]` と `facilityContents: FacilityContent[]` を足す(req.tsp:438 付近)。scalar は既存(EntityID/EntryWeight/Dice)を再利用し、幅・null 規約は ruins-tsp に従う。required 配列は producer が空スライスで初期化する。
...
挙動不変の移植なので golden は変えない。ロードした Content が現行 Go Content と同一構造・同一順序になるよう1対1で移植し、抽選乱数の消費順を変えない。移植後に `make check` で interior の distribution/fill/sprite golden が不変であることを確認する。ずれたら移植の取りこぼしなので golden を再生成せず移植を直す。
...
| `assets/metadata/entities/raw/raw.toml` | `[[interiorContents]]` 17レシピ+部屋、`[[facilityContents]]` 施設→変種。yq で編集、id 衝突を事前 grep |
...
- [ ] `content_catalog.go` の17レシピ+部屋を raw.toml へ移植。yq、id 衝突 grep
- [ ] `facilityVariants` を `facilityContents` データへ移し、Go 側を削除
