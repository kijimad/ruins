...
- [x] `FurnishBuilding`/`Furnish`/`FurnishStages` に `*ContentSet` を渡す。当初は波及を避けて `activeContents()` の package 既定にしたが、mapplanner が raw.toml を独立ロードする二重読込を嫌い、引数で渡す形へ戻した。本番は `world.Resources.RawMaster` と balance の `master` から `LoadContents` で組み、テストは共有の `testContents()` から渡す。mapplanner は raw.toml を読まなくなった
