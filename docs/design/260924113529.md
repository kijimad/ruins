...
- [ ] tsp に enum と model を追加し `Raws` へ3フィールド(interiorContents/facilityContents/facilityRooms)。`make genall` で生成
- [ ] `LoadContents` で oapi→interior.Content 変換を実装。Satellites は archetype 補完で空
- [ ] `LoadContents` の変換等価テスト。ID・Groups・Items の順序込みで現行 Go Content と一致することを golden に頼らず固定
