...
`saveOnce` は入れ子の Guard に対する不変条件を担う。`Update` の Guard が再 panic し、それが同一
goroutine の上位 Guard に再捕捉されても、保存は最初の1回だけ走りファイルは1つに保たれる。Ebiten が
`Update` を別 goroutine で回す場合は上位 Guard に届かず、そのまま全体終了する。どちらの goroutine
モデルでも「クラッシュ1件＝ファイル1つ」を守る。`runPlay` 頂上の Guard は主に初期化フェーズの
panic を受け持つ。
