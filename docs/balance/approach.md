...
- 感度と索引: ドメイン横断の感度行列（つまみ×メトリクスの弾力性、`CrossDomainSensitivity`）を baseline.md に出す。つまみは `knobRegistry` の全成分を機械的に回すので、Params に成分を足せば行列へ自動で載る。ほぼブロック対角でドメインは疎結合、横断するのは1日ターン数のような共通分母だけ、と可視化する。外れをどのつまみで戻すかの手掛かりになる。
...
- 運用: パラメータ調整は baseline.md の目標帯ダッシュボードと感度行列、差分、凍結ゲートで回す。目標帯を人間が決め、感度行列と交換レートでつまみを引き当て、探索や手編集で値を寄せる。
