...
- 感度と索引: ドメイン横断の感度行列（つまみ×メトリクスの弾力性、`CrossDomainSensitivity`）を baseline.md に出す。ほぼブロック対角でドメインは疎結合、横断するのは1日ターン数のような共通分母だけ、と可視化する。外れをどのつまみで戻すかの手掛かりになる。全パラメータの所在と編集経路は `docs/balance/tuning.md` に索引化した。
