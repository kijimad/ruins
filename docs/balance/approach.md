...
- スキル深度: `balance/skilldepth.go` の `SkillDepthProfileFor` はスキルを1レベル上げるたびに撃破ターンがどれだけ縮むかを測り、丸めで武器ダメージが変わらない死んだティアと、体験が動く実効ティアに分ける。NTBEA のティア識別性を対戦でなく体験メトリクスへ翻案したもの。熟練度は+5%/レベルだが武器ダメージは整数丸めなので、弱い武器ほど死んだティアが多く進行が平坦になる。素手は100段中85段が死に、鉄剣は25段。baseline.md に載る。
