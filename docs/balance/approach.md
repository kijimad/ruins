...
- 残る発展: 探索1回の総収支。1個あたりの期待手取りは閉形式で出したが、総収支はこれに拾える個数と移動燃料を掛ける。1回の移動タイル数はコードにない設計値なので閉形式では導けず、モンテカルロと設計値の領域に残す。定数側を目標へ寄せる摂動窓つきの感度行列も次の候補。
- 運用: パラメータ調整は baseline.md の目標帯ダッシュボードと差分、凍結ゲートで回す。目標帯を人間が決め、探索や手編集で値を寄せる。多目的Pareto探索は複数日のトレードオフを非劣集合で見せる。
