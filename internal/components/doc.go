...
// - 相互作用の種類はInteractionKind（Door, Talk, Item, Melee等）で判別する
...
// - Kind==InteractionMeleeは敵が「攻撃可能な対象」であることを示す
//
// コンポーネント設計指針:
//   - フィールド観察の敵味方など派生的な分類は、コンポーネントを足さず Categories 定義で吸収する
//   - 「それが何であるか」を十分に絞って示す用途には、タグコンポーネントを追加する
//     例: Tile, Player はエンティティの本質的な種別を示すタグコンポーネント
//
