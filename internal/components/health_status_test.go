...
func TestHealthStatus_Capacities(t *testing.T) {
	...
	t.Run("不調なしは全機能100で痛み0", func(t *testing.T) {
		t.Parallel()
		caps := (&HealthStatus{}).BodyFunctions()
		assert.Equal(t, BodyFunctions{Pain: 0, Blood: 100, Consciousness: 100, Manipulation: 100, Moving: 100, Sight: 100}, caps)
	})
	...
	t.Run("腕の骨折は操作を下げ痛みを与え意識を落とす", func(t *testing.T) {
		...
		caps := hs.BodyFunctions()
		// 骨折 18/20 の中度。痛み=18*2=36、意識=100-36/2=82、操作=(100-20*2)*82/100=49、
		// 歩行と視覚は局所低下なしだが意識が掛かって82
		assert.Equal(t, BodyFunctions{Pain: 36, Blood: 100, Consciousness: 82, Manipulation: 49, Moving: 82, Sight: 82}, caps)
	})
...
func TestBodyPartMetas_全部位が登録されている(t *testing.T) {
	...
	for bp := range BodyPartCount {
		...
		assert.NotEmpty(t, bodyPartMetas[bp].bodyFunction, "部位 %d に身体機能がある", bp)
	}
}
...
func TestHealthStatus_BloodLoss(t *testing.T) {
	...
	t.Run("重度の切り傷は血液量を下げ HP を減らす", func(t *testing.T) {
		...
		assert.Equal(t, 25, int(hs.BodyFunctions().Blood), "失血75で血液量25")
	...
	t.Run("中度の切り傷では血液量が保たれ HP は減らない", func(t *testing.T) {
		...
		assert.Equal(t, 50, int(hs.BodyFunctions().Blood))
	...
	t.Run("治療で血液量が戻り失血が止まる", func(t *testing.T) {
		...
		assert.Equal(t, 100, int(hs.BodyFunctions().Blood))
		drain, _ := hs.BloodLossHPDrain()
...
func TestConditionBodyFunctionImpact(t *testing.T) {
	t.Parallel()

	t.Run("腕の骨折は操作を下げ痛みを与える", func(t *testing.T) {
		t.Parallel()
		// 骨折 18/20 の中度: 痛み 18*2=36、操作 20*2=40
		pain, bodyFunction, drop := ConditionBodyFunctionImpact(&HealthCondition{Type: ConditionFracture, Severity: SeverityMedium}, BodyPartArms)
		assert.Equal(t, 36, pain)
		assert.Equal(t, BodyFuncManipulation, bodyFunction)
	...
	t.Run("応急処置で痛みと機能低下が半減する", func(t *testing.T) {
		...
		treatedPain, _, treatedDrop := ConditionBodyFunctionImpact(&HealthCondition{Type: ConditionFracture, Severity: SeverityMedium, TendQuality: 100}, BodyPartArms)
	...
	t.Run("症状ごとに反応率が違う", func(t *testing.T) {
		...
		fracPain, _, fracDrop := ConditionBodyFunctionImpact(&HealthCondition{Type: ConditionFracture, Severity: SeverityMedium}, BodyPartArms)
		lacPain, _, lacDrop := ConditionBodyFunctionImpact(&HealthCondition{Type: ConditionLaceration, Severity: SeverityMedium}, BodyPartArms)
	...
	t.Run("脚の不調は歩行を下げる", func(t *testing.T) {
		t.Parallel()
		_, bodyFunction, _ := ConditionBodyFunctionImpact(&HealthCondition{Type: ConditionFracture, Severity: SeverityMinor}, BodyPartFeet)
		assert.Equal(t, BodyFuncMoving, bodyFunction)
	})

	t.Run("重症度なしは影響なし", func(t *testing.T) {
		t.Parallel()
		// bodyFunction は部位で定まり重症度に依らない。影響なしは drop と pain が0であることで表す
		pain, bodyFunction, drop := ConditionBodyFunctionImpact(&HealthCondition{Type: ConditionFracture, Severity: SeverityNone}, BodyPartArms)
