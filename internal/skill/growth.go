...
const (
	baseExp       = 10 // 基本獲得経験値
	attrBonus     = 5  // 属性値1あたりの成長速度ボーナス（%）
	decayPerLevel = 20 // スキル値1あたりの減衰率（%）
)

// GainExp はスキルに経験値を加算する。スキルアップしたらtrueを返す。
// attributeValueは対応する属性値で、高いほど獲得経験値が増える。
// 式: exp = baseExp * (100 + attrValue*5) / 100 * 100 / (100 + currentValue*20)
func GainExp(s *gc.Skill, attributeValue int) bool {
	growthSpeed := 100 + attributeValue*attrBonus
