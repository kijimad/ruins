...
//   - BuildWorld: ステートを起動して地形やエンティティまで用意したワールドを返す。描画はしない。
//     Render3DSystem などステート外のレンダラや検査に使う
//
...
//   - AssertTopStateGolden: 最上段ステートだけを描いて固定する。下段はワールドのデータ供給に積み、
//     ワールドは映さない。UIがワールドの上に載るがワールドを見せたくないときに使う
//
