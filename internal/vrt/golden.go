...
//
// 960×720 の全画面ステートで約0.012%、約83画素、になる値にしている。VRT は
// Config.DisableScreenFilter でレトロフィルタを切って撮る。スキャンラインは全画面の高周波
// パターンで、フォントのアンチエイリアスやアルファ合成の GL 実装差をピクセル比較上で増幅して
// いた。切ったあとの実測は、並列・シャッフルで回しても差分画素0、振幅も8bitで1階調未満。
// 0.012%はそこへ実行環境差ぶんの余裕を持たせた値。
//
// 検出できる粒度: メニューのラベル2行を書き換えた実測が0.0825%、570画素、で検出する。
// 逆に句読点1文字ぶん、16画素で0.0023%、は下回るので検出しない。文字単位の変化まで
// 見たいならトレランスではなく専用の小さい golden を撮る。
//
// この係数を緩めると実変化が静かに素通りする。以前の45.0は全画面で5.4%、400×120の
// ウィジェットでは20.5%あり、テキスト行が丸ごと別言語になっていた golden を見逃していた。
