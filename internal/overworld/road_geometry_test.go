...
func TestRoadSegments_水平辺は始点の行_垂直辺は終点の列(t *testing.T) {
	...
	assert.Equal(t, roadSeg{orient: orientHorizontal, fixed: 0, lo: 0, hi: 2}, segs[0], "水平辺は始点 a の行 a.Y、列は a.X..b.X")
	assert.Equal(t, roadSeg{orient: orientVertical, fixed: 2, lo: 0, hi: 3}, segs[1], "垂直辺は終点 b の列 b.X、行は a.Y..b.Y。角は (b.X, a.Y)")
}
...
func TestRoadSegments_端点は大小に依らず正規化される(t *testing.T) {
	...
	assert.Equal(t, roadSeg{orient: orientHorizontal, fixed: 3, lo: 1, hi: 4}, segs[0], "水平辺の列は min/max で正規化")
	assert.Equal(t, roadSeg{orient: orientVertical, fixed: 1, lo: 1, hi: 3}, segs[1], "垂直辺の行は min/max で正規化")
}
...
func TestRoadSegTileSpan_固定軸と可変軸を正しいチャンク寸法で中心へ寄せる(t *testing.T) {
	...
	hFixed, hLo, hHi := roadSeg{orient: orientHorizontal, fixed: 2, lo: 0, hi: 3}.tileSpan(chunkW, chunkH)
	...
	vFixed, vLo, vHi := roadSeg{orient: orientVertical, fixed: 2, lo: 0, hi: 3}.tileSpan(chunkW, chunkH)
