...
func TestBuildMacroView_北進帯は絶対チャンク行をnorthIndexでずらす(t *testing.T) {
	...
	view := BuildMacroView(testutil.InitTestWorld(t).Resources.RawMaster, 1, northIndex, chunkW, chunkH, area, player, true, cubes, nil)
...
func TestBuildMacroView_表示範囲の格子とマーカーを表示範囲ローカルへ組む(t *testing.T) {
	...
	view := BuildMacroView(testutil.InitTestWorld(t).Resources.RawMaster, 1, 0, 10, 10, area, player, true, cubes, nil)
...
func TestBuildMacroView_フォグは探索済みチャンクだけ開放する(t *testing.T) {
	...
	view := BuildMacroView(testutil.InitTestWorld(t).Resources.RawMaster, 1, 0, 10, 10, area, consts.Coord[consts.Tile]{}, false, nil, discovered)
...
func TestBuildMacroView_道の接続方角をセルに埋める(t *testing.T) {
	...
	view := BuildMacroView(testutil.InitTestWorld(t).Resources.RawMaster, runSeed, 0, 10, 10, area, consts.Coord[consts.Tile]{}, false, nil, nil)
...
func TestBuildMacroView_表示範囲外のマーカーは落とす(t *testing.T) {
	...
	view := BuildMacroView(testutil.InitTestWorld(t).Resources.RawMaster, 1, 0, 10, 10, area, player, true, []consts.Coord[consts.Tile]{{X: 99, Y: 99}}, nil)
...
func TestBuildMacroView_プレイヤー不在なら現在地なし(t *testing.T) {
	t.Parallel()
	area := MacroRange{OriginX: 0, Cols: 2, Rows: 1}
	view := BuildMacroView(testutil.InitTestWorld(t).Resources.RawMaster, 1, 0, 10, 10, area, consts.Coord[consts.Tile]{}, false, nil, nil)
