...
func TestFullBandRange_帯全体に東西の余白を足す(t *testing.T) {
	t.Parallel()
	rng := FullBandRange(10, 4, 3)
	assert.Equal(t, 10-MacroMargin, rng.OriginX, "表示範囲の左端は東進位置から余白ぶん西")
	assert.Equal(t, 4+2*MacroMargin, rng.Cols, "列数は帯幅に東西の余白を足す")
	assert.Equal(t, consts.Chunk(3), rng.Rows, "行数は帯の高さ")
}

func TestBuildMacroView_表示範囲の格子とマーカーを表示範囲ローカルへ組む(t *testing.T) {
	t.Parallel()
	// eastIndex=0, 1チャンク=10タイル。表示範囲は絶対列0から3列×2行
	rng := MacroRange{OriginX: 0, Cols: 3, Rows: 2}
	player := consts.Coord[consts.Tile]{X: 15, Y: 5}     // チャンク(1,0)
	cubes := []consts.Coord[consts.Tile]{{X: 25, Y: 12}} // チャンク(2,1)
	view := BuildMacroView(1, 0, 10, 10, rng, player, true, cubes, nil)
...
func TestBuildMacroView_フォグは探索済みチャンクだけ開放する(t *testing.T) {
	t.Parallel()
	rng := MacroRange{OriginX: 0, Cols: 3, Rows: 1}
	// 絶対チャンク(1,0)だけ開放済みにする
	discovered := map[consts.Coord[consts.Chunk]]bool{{X: 1, Y: 0}: true}
	view := BuildMacroView(1, 0, 10, 10, rng, consts.Coord[consts.Tile]{}, false, nil, discovered)
...
func TestBuildMacroView_道の接続方角をセルに埋める(t *testing.T) {
	...
	view := BuildMacroView(runSeed, 0, 10, 10, rng, consts.Coord[consts.Tile]{}, false, nil, nil)
...
func TestBuildMacroView_表示範囲外のマーカーは落とす(t *testing.T) {
	t.Parallel()
	rng := MacroRange{OriginX: 0, Cols: 2, Rows: 1}
	player := consts.Coord[consts.Tile]{X: 55, Y: 5} // チャンク(5,0)。表示範囲の外
	view := BuildMacroView(1, 0, 10, 10, rng, player, true, []consts.Coord[consts.Tile]{{X: 99, Y: 99}}, nil)
...
func TestBuildMacroView_プレイヤー不在なら現在地なし(t *testing.T) {
	t.Parallel()
	rng := MacroRange{OriginX: 0, Cols: 2, Rows: 1}
	view := BuildMacroView(1, 0, 10, 10, rng, consts.Coord[consts.Tile]{}, false, nil, nil)
