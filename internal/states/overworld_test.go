...
func TestOverworldState_ロード復元で視界が再計算され真っ暗にならない(t *testing.T) {
	...
	factory := NewOverworldState(mapplanner.PlannerTypeOverworldField, &NewGameParams{RunSeed: 777, ChunkW: chunkW, ChunkH: chunkH, K: 3})
	state, err := factory()
...
func TestOverworldState_OnStart_初期帯とプレイヤー中央(t *testing.T) {
	...
	factory := NewOverworldState(mapplanner.PlannerTypeSmallRoom, &NewGameParams{RunSeed: 777, ChunkW: chunkW, ChunkH: chunkH, K: k})
	state, err := factory()
	require.NoError(t, err)
	st, ok := state.(*DungeonState)
	require.True(t, ok)
	...
	// 開始チャンクにダンジョン入口が1つ置かれ、進入先のダンジョン名を持つ
	entranceCount := 0
	eq := ecs.NewFilter1[gc.DungeonEntrance](world.ECS).Query()
	for eq.Next() {
		entranceCount++
		assert.NotEmpty(t, world.Components.DungeonEntrance.Get(eq.Entity()).DefinitionName, "ダンジョン入口は進入先を持つ")
		assert.True(t, world.Components.Interactable.Has(eq.Entity()), "ダンジョン入口は相互作用を持つ")
	}
	assert.Equal(t, 1, entranceCount, "開始チャンクにダンジョン入口が1つ置かれる")
}
...
func TestOverworldState_オーバーレイ進入で帯タイルを消さない(t *testing.T) {
	...
	factory := NewOverworldState(mapplanner.PlannerTypeSmallRoom, &NewGameParams{RunSeed: 777, ChunkW: chunkW, ChunkH: chunkH, K: 3})
	state, err := factory()
	require.NoError(t, err)
	st, ok := state.(*DungeonState)
	require.True(t, ok)
