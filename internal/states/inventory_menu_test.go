...
func TestInventoryMenuState_WindowProps(t *testing.T) {
	...
	// 初期状態ではウィンドウは閉じている
	windowProps := state.windowMount.GetProps()
	assert.False(t, windowProps.Open, "初期状態ではウィンドウは閉じている")
	assert.Equal(t, uint64(0), uint64(windowProps.SelectedEntity), "初期状態ではエンティティは0")
}
...
func TestInventoryMenuState_DoAction_WindowMode(t *testing.T) {
	...
	// ウィンドウが閉じている
	windowProps := state.windowMount.GetProps()
	assert.False(t, windowProps.Open, "キャンセル後はウィンドウが閉じている")
}
