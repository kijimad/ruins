...
func TestPhaseG_遺跡滞在中にセーブロードしても共存が復元され地上へ戻れる(t *testing.T) {
	...
	sess := overworld.NewSession(mapplanner.PlannerTypeOverworldField, dungeon.NewOverworldDefinition("オーバーワールド", 0, 30, 20, 3), &overworld.NewGameParams{RunSeed: 42})
...
func TestPhaseG_多層の共存がセーブロードを跨いで保持され順に地上まで戻れる(t *testing.T) {
	...
	sess := overworld.NewSession(mapplanner.PlannerTypeOverworldField, dungeon.NewOverworldDefinition("オーバーワールド", 0, 30, 20, 3), &overworld.NewGameParams{RunSeed: 7})
	require.NoError(t, sess.Start(world))
...
func TestPhaseG_遺跡から地上へ戻ると帯寸法と視界が復元され隊員も配置される(t *testing.T) {
	...
	sess := overworld.NewSession(mapplanner.PlannerTypeOverworldField, dungeon.NewOverworldDefinition("オーバーワールド", 0, 50, 50, 3), &overworld.NewGameParams{RunSeed: 1})
	require.NoError(t, sess.Start(world))
	player, err := query.GetPlayerEntity(world)
	require.NoError(t, err)
...
func TestPhaseG_遺跡内で保存しロード復元しても現ステージが遺跡のまま(t *testing.T) {
	t.Parallel()
	world := testutil.InitTestWorld(t)
	sess := overworld.NewSession(mapplanner.PlannerTypeOverworldField, dungeon.NewOverworldDefinition("オーバーワールド", 0, 30, 20, 3), &overworld.NewGameParams{RunSeed: 3})
	require.NoError(t, sess.Start(world))
...
func TestDebug_遺跡進入イベントで正規経路を通って遺跡へ入る(t *testing.T) {
	t.Parallel()
	world := testutil.InitTestWorld(t)
	sess := overworld.NewSession(mapplanner.PlannerTypeOverworldField, dungeon.NewOverworldDefinition("オーバーワールド", 0, 30, 20, 3), &overworld.NewGameParams{RunSeed: 9})
	require.NoError(t, sess.Start(world))
...
func TestDebug_プランナー指定進入で指定プランナーが固定される(t *testing.T) {
	t.Parallel()
	world := testutil.InitTestWorld(t)
	sess := overworld.NewSession(mapplanner.PlannerTypeOverworldField, dungeon.NewOverworldDefinition("オーバーワールド", 0, 30, 20, 3), &overworld.NewGameParams{RunSeed: 11})
	require.NoError(t, sess.Start(world))
...
func TestDebug_プランナー指定は選ぶたびにフロアを作り直す(t *testing.T) {
	t.Parallel()
	world := testutil.InitTestWorld(t)
	sess := overworld.NewSession(mapplanner.PlannerTypeOverworldField, dungeon.NewOverworldDefinition("オーバーワールド", 0, 30, 20, 3), &overworld.NewGameParams{RunSeed: 13})
	require.NoError(t, sess.Start(world))
	st := &DungeonState{DefinitionName: dungeon.DungeonOverworld.Name()}
	ruinKey := gc.NewDungeonStage(dungeon.DungeonDebug.Name(), 1)
