...
// WarpDungeonEnterWithBuilderEvent→enterDebugPlannerFloor を通り、指定プランナーで生成することを固定する。
func TestDebug_プランナー指定進入で指定プランナーが固定される(t *testing.T) {
	...
	require.NoError(t, lifecycle.RequestStateChange(world, gc.WarpDungeonEnterWithBuilderEvent(dungeon.DungeonDebug.Name(), mapplanner.PlannerTypeBigRoom.Name)))
	...
	require.NoError(t, lifecycle.RequestStateChange(world, gc.WarpDungeonEnterWithBuilderEvent(dungeon.DungeonDebug.Name(), "存在しないプランナー")))
	_, err = st.handleStateChangeRequest(world)
...
func TestDebug_プランナー指定は選ぶたびにフロアを作り直す(t *testing.T) {
	...
	require.NoError(t, lifecycle.RequestStateChange(world, gc.WarpDungeonEnterWithBuilderEvent(dungeon.DungeonDebug.Name(), mapplanner.PlannerTypeSmallRoom.Name)))
	_, err = st.handleStateChangeRequest(world)
	require.NoError(t, err)
