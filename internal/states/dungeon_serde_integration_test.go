...
func TestPhaseG_遺跡から地上へ戻ると帯寸法と視界が復元され隊員も配置される(t *testing.T) {
	...
	st2 := &DungeonState{Depth: query.GetDungeon(world).Depth}
	...
	sb := d.SeamlessBand
	assert.Equal(t, sb.K.Tiles(sb.ChunkW), d.Level.TileWidth, "帯幅の Level が復元される")
	assert.Equal(t, sb.ChunkH, d.Level.TileHeight, "帯高さの Level が復元される")
	assert.True(t, d.NeedsForceUpdate, "視界の強制再計算が要求される")
...
// ロード復元したとき、現ステージが遺跡のまま保たれることを固定する。共存方式では遺跡滞在中も
// SeamlessBand.Active なので、復元時に無条件でオーバーワールドへ書き換えると現在地が壊れ、
// 遺跡なのに前線の霜が誤って描かれた。frostVisible は現ステージで判定するため、これが要となる。
func TestPhaseG_遺跡内で保存しロード復元しても現ステージが遺跡のまま(t *testing.T) {
	...
	// 遺跡へ入る。現ステージは遺跡、帯は Active のまま退避される。
	st := &DungeonState{DefinitionName: dungeon.DungeonOverworld.Name}
	require.NoError(t, st.enterDungeon(world, dungeon.DungeonDebug.Name))
	ruinKey := gc.NewNamedDungeonStage(dungeon.DungeonDebug.Name, 1)
	...
	// ロード復元。SeamlessBand.Active なので OverworldState で復帰し Session.Start を呼ぶ。
	require.True(t, query.GetDungeon(newWorld).SeamlessBand.Active, "帯は Active のまま復元される")
	resume := overworld.NewSession(mapplanner.PlannerTypeOverworldField, nil)
	require.NoError(t, resume.Start(newWorld))

	// 復元後も現ステージは遺跡のまま。オーバーワールドへ書き換わらないので前線の霜も出ない。
