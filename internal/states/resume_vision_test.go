...
func TestDungeonResume_視界を強制再計算する(t *testing.T) {
	...
	query.GetVisionState(world).PendingUpdate = gc.VisionUpdateNone

	st := &gs.DungeonState{Depth: 3, DefinitionName: dungeon.DungeonDebug.Name(), Resume: true}
	require.NoError(t, st.OnStart(world))

	assert.Equal(t, gc.VisionUpdateForce, query.GetVisionState(world).PendingUpdate, "通常ダンジョンの復帰は視界を強制再計算する")
}
...
func TestOverworldResume_視界を強制再計算する(t *testing.T) {
	...
	assert.Equal(t, gc.VisionUpdateForce, query.GetVisionState(world).PendingUpdate, "オーバーワールド開始も視界を強制再計算する")
}
