...
func healthDetailContent(world w.World, item statusItemData) overlay.DetailContent {
	...
	pain, bodyFunction, drop := gc.ConditionBodyFunctionImpact(cond, item.BodyPart)
	...
	if drop > 0 {
		rows = append(rows, entityspec.SpecRow{Label: query.T(world, string(bodyFunction)), Value: fmt.Sprintf("-%d", drop)})
	}
