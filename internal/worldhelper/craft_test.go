...
func TestCanCraft(t *testing.T) {
	...
	material, _ := SpawnItem(world, "木の棒", 5, gc.LocationTypeInBackpack)
	...
	// 素材が不足している場合のテスト
	materialComp := world.Components.Item.Get(material).(*gc.Item)
...
func TestCraft(t *testing.T) {
	...
	// 素材を用意してクラフト成功
	_, _ = SpawnItem(world, "木の棒", 5, gc.LocationTypeInBackpack)
	result, err = Craft(world, "木刀")
