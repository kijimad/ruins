...
func buildDetailUI(res resources.UIResources, rect image.Rectangle, name, desc string, rows []entityspec.SpecRow, page int, bodyDesc bool) uicore.Widget {
	...
	// 説明は既定では補助色の小フェイスだが、本文として読ませたいときは本文フェイスと主色にする
	descFace := smallFace
	descColor := theme.TextSecondary
	if bodyDesc {
		descFace = face
		descColor = theme.TextPrimary
	}
	...
	if desc != "" && page == total-1 {
		for _, line := range uicore.WrapText(desc, descFace, rect.Dx()-theme.Space7*2) {
			items = append(items, uicore.NewText(line, descFace, descColor))
		}
	}
