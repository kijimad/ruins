...
// buildMu はebitenuiのグローバルなイベントハンドラ登録が並行アクセス安全でないため、
// widget.Container.AddChild を伴うウィジェット構築呼び出しを直列化する。
// t.Parallel() 自体は維持しつつ、構築処理だけをロックで守る
var buildMu sync.Mutex

// updateSpec はロックを取った上で views.UpdateSpec を呼ぶ
func updateSpec(world w.World, root *widget.Container, e ecs.Entity) {
	buildMu.Lock()
	defer buildMu.Unlock()
	views.UpdateSpec(world, root, e)
}

// updateSpecFromSpec はロックを取った上で views.UpdateSpecFromSpec を呼ぶ
func updateSpecFromSpec(world w.World, root *widget.Container, spec gc.EntitySpec) {
	buildMu.Lock()
	defer buildMu.Unlock()
...
func TestUpdateSpecFromSpec_エンティティを生成せずに近接武器の性能を表示する(t *testing.T) {
	...
	updateSpecFromSpec(world, root, spec)
	labels := collectLabels(root)
