...
func TestRemoveEntitiesInYRange(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	// 範囲外の敵 → 残る
	outside, err := lifecycle.SpawnEnemy(world, consts.Coord[consts.Tile]{X: 15, Y: 15}, "fireball")
	require.NoError(t, err)

	removed := worldstream.RemoveEntitiesInYRange(world, 0, 10, worldstream.KeepPlayer(world))
...
func TestRemoveEntitiesInYRange_境界は半開区間(t *testing.T) {
	t.Parallel()

	world := testutil.InitTestWorld(t, testutil.WithStageLevel(gc.Level{TileWidth: consts.Tile(100), TileHeight: consts.Tile(60)}))
	if _, err := lifecycle.SpawnPlayer(world, consts.Coord[consts.Tile]{X: 5, Y: 50}, "ash"); err != nil {
		require.NoError(t, err)
	}

	atLo, err := lifecycle.SpawnEnemy(world, consts.Coord[consts.Tile]{X: 5, Y: 0}, "fireball") // lo は含む
	require.NoError(t, err)
	atHi, err := lifecycle.SpawnEnemy(world, consts.Coord[consts.Tile]{X: 5, Y: 10}, "fireball") // hi は含まない
	require.NoError(t, err)

	removed := worldstream.RemoveEntitiesInYRange(world, 0, 10, nil)

	assert.Equal(t, 1, removed, "[0,10) の半開区間。Y=0 は含み Y=10 は含まない")
	assert.False(t, world.ECS.Alive(atLo), "Y=lo は範囲内で削除")
	assert.True(t, world.ECS.Alive(atHi), "Y=hi は範囲外で残る")
}

// TestRemoveEntitiesInYRange_所有者の収納在庫も道連れにする は、破棄される所有者が収納に持つ実体も
// 一緒に消えることを固定する。収納在庫は GridElement を持たず座標カリングでは拾われないため、
// 所有者だけ消えて在庫が孤児化するのを防ぐ。
func TestRemoveEntitiesInYRange_所有者の収納在庫も道連れにする(t *testing.T) {
	t.Parallel()
	...
	// 所有者を含む範囲を破棄する
	worldstream.RemoveEntitiesInYRange(world, 0, 10, nil)

	assert.False(t, world.ECS.Alive(owner), "所有者が消える")
	assert.False(t, world.ECS.Alive(stock), "収納在庫も道連れで消える")
}

// TestRemoveEntitiesInYRange_範囲外の所有者の在庫は残す は、破棄されない所有者の収納在庫は
// 消さないことを固定する。所有者ごとに在庫の可否を分ける。
func TestRemoveEntitiesInYRange_範囲外の所有者の在庫は残す(t *testing.T) {
	t.Parallel()

	world := testutil.InitTestWorld(t, testutil.WithStageLevel(gc.Level{TileWidth: consts.Tile(100), TileHeight: consts.Tile(60)}))

	owner := world.ECS.NewEntity()
	world.Components.GridElement.Add(owner, &gc.GridElement{Coord: consts.Coord[consts.Tile]{X: 5, Y: 50}}) // 範囲外
	stock, err := lifecycle.SpawnStorageItem(world, "wooden_sword", 1, owner)
	require.NoError(t, err)
