...
func TestBand_ShiftEast(t *testing.T) {
	...
	gen := func(chunkIndex int, offsetX consts.Tile) error {
	...
	assert.Equal(t, 3, gotChunkIndex, "新チャンクの絶対インデックス")
	...
	// 揮発キャッシュはクリアされている
	assert.Empty(t, d.VisibleTiles, "VisibleTiles はクリアされ次フレーム再構築")
}
...
func TestBand_ShiftWest(t *testing.T) {
	...
	player, err := lifecycle.SpawnPlayer(world, 90, 30, "Ash")
	require.NoError(t, err)
	...
	assert.Equal(t, 0, gotChunkIndex, "新しい西端チャンクの絶対インデックス")
