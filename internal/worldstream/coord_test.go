...
func TestBandOriginX(t *testing.T) {
	...
	assert.Equal(t, worldstream.AbsTileX(0), worldstream.BandOriginX(0, 100), "eastIndex=0 は原点0")
	assert.Equal(t, worldstream.AbsTileX(300), worldstream.BandOriginX(3, 100), "eastIndex*chunkW")
}
...
func TestAbsLocalRoundTrip(t *testing.T) {
	...
	assert.Equal(t, worldstream.AbsTileX(237), abs, "ローカル→絶対はオフセット加算")
...
func TestToLocal_前線を帯ローカルへ(t *testing.T) {
	...
	frontEast := worldstream.AbsTileX(540)    // 帯の40タイル目に前線がある
