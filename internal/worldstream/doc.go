...
// ストリーミングするための基盤を提供する。
//
// K 個の隣接チャンクを1連続座標空間に並べた「単一マップ」（帯）を管理し、プレイヤーが
// 中央チャンクを東へ出るとシフト（東端生成・西端破棄・座標リベース）する。これにより帯ローカル
...
//   - Band: 帯の状態とシフト（ShiftEast/ShiftWest）
//
// mapplanner/mapspawner には依存しない（チャンク生成は ChunkGen 注入で分離）。実生成の
