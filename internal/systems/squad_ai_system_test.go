...
func TestSquadProcessor_護衛ポリシーでリーダーに追従する(t *testing.T) {
	...
	// リーダーに近づいているはず
	finalGrid := world.Components.GridElement.MustGet(member)
...
func TestSquadProcessor_待機ポリシーで移動しない(t *testing.T) {
	...
	ai.Planner.(*gc.SquadAI).Movement = gc.SquadStationary
...
func TestSquadProcessor_回避ポリシーで敵から離れる(t *testing.T) {
	...
	// 回避ポリシーに変更する
	ai := world.Components.AI.MustGet(member)
	ai.Planner.(*gc.SquadAI).CombatCurrent = gc.CombatEvade
...
func TestSquadProcessor_前衛ポリシーでリーダーから離れすぎると接近する(t *testing.T) {
	...
	// 前衛ポリシーに変更する
	ai := world.Components.AI.MustGet(member)
	ai.Planner.(*gc.SquadAI).Movement = gc.SquadVanguard
...
func TestSquadProcessor_巡回ポリシーで移動する(t *testing.T) {
	...
	// 巡回ポリシーに変更する
	ai := world.Components.AI.MustGet(member)
	ai.Planner.(*gc.SquadAI).Movement = gc.SquadPatrol
...
func TestSquadProcessor_CombatIgnoreで戦闘しない(t *testing.T) {
	...
	enemy, err := lifecycle.SpawnEnemy(world, 12, 10, "火の玉")
	require.NoError(t, err)
	enemyAI := world.Components.AI.MustGet(enemy)
	enemySolo := enemyAI.Planner.(*gc.SoloAI)
