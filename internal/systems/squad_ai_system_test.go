...
func TestSquadProcessor_待機ポリシーで移動しない(t *testing.T) {
	...
	// 待機ポリシーに変更する
	world.Components.SquadAI.Get(member).(*gc.SquadAI).Movement = gc.SquadStationary
...
func TestSquadProcessor_死亡した隊員は処理されない(t *testing.T) {
	...
	// 死亡状態にする
	member.AddComponent(world.Components.Dead, &gc.Dead{})
...
func TestSquadProcessor_回避ポリシーで敵から離れる(t *testing.T) {
	...
	// 回避ポリシーに変更する
	world.Components.SquadAI.Get(member).(*gc.SquadAI).CombatCurrent = gc.CombatEvade
	...
	// 隊員の近くに敵を配置する
	_, err = lifecycle.SpawnEnemy(world, 13, 10, "火の玉")
	require.NoError(t, err)
...
func TestSquadProcessor_前衛ポリシーでリーダーから離れすぎると接近する(t *testing.T) {
	...
	// 前衛ポリシーに変更する
	world.Components.SquadAI.Get(member).(*gc.SquadAI).Movement = gc.SquadVanguard
...
func TestSquadProcessor_巡回ポリシーで移動する(t *testing.T) {
	...
	// 巡回ポリシーに変更する
	world.Components.SquadAI.Get(member).(*gc.SquadAI).Movement = gc.SquadPatrol
...
func TestSquadProcessor_CombatIgnoreで戦闘しない(t *testing.T) {
	...
	// CombatIgnoreに変更する
	squad := world.Components.SquadAI.Get(member).(*gc.SquadAI)
	...
	enemy, err := lifecycle.SpawnEnemy(world, 12, 10, "火の玉")
	require.NoError(t, err)
	enemySolo := world.Components.SoloAI.Get(enemy).(*gc.SoloAI)
