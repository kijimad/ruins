...
func TestSquadProcessor_待機ポリシーで移動しない(t *testing.T) {
	...
	ai.Planner.(*gc.SquadAI).Movement = gc.MovementStationary
...
func TestSquadProcessor_前衛ポリシーでリーダーから離れすぎると接近する(t *testing.T) {
	...
	// 前衛ポリシーに変更する
	ai := world.Components.AI.Get(member).(*gc.AI)
	ai.Planner.(*gc.SquadAI).Movement = gc.MovementVanguard
...
func TestSquadProcessor_巡回ポリシーで移動する(t *testing.T) {
	...
	// 巡回ポリシーに変更する
	ai := world.Components.AI.Get(member).(*gc.AI)
	ai.Planner.(*gc.SquadAI).Movement = gc.MovementPatrol
