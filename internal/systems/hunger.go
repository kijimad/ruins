...
// 呼ぶので、移動や射撃をしないその場待機のエンティティも等しく空腹になる。行動種別に紐づけると動くものだけ空腹になり
// 不公平になるため、活動側でなくここへ集約する。
//
// ゲートは共有 RNG を使わず、entity と turn から決定的な擬似乱数を作る。共有 RNG を消費するとエンティティ数ぶん敵AIや
...
func progressTurnHunger(world w.World) {
	...
	for q.Next() {
		entity := q.Entity()
		hungerPct := int(query.Modifiers(world, entity).Value(gc.ModHungerProgress))
