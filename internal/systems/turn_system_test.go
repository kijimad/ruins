...
func TestTurnSystem_Update(t *testing.T) {
	...
	t.Run("PlayerTurnでAPがマイナスなら自動でAITurnへ遷移", func(t *testing.T) {
		...
		turnManager.TurnPhase = turns.PlayerTurn
		...
		assert.Equal(t, turns.AITurn, turnManager.TurnPhase, "APがマイナスならAITurnへ遷移するべき")
	})
	...
	t.Run("PlayerTurnでAPが0以上なら遷移しない", func(t *testing.T) {
		...
		assert.Equal(t, turns.PlayerTurn, turnManager.TurnPhase, "APが0以上ならPlayerTurnのまま")
	})
	...
	t.Run("AITurnからTurnEndへ遷移", func(t *testing.T) {
		...
		assert.Equal(t, turns.TurnEnd, turnManager.TurnPhase, "AITurnからTurnEndへ遷移するべき")
	})
	...
	t.Run("TurnEndから新しいターンへ遷移", func(t *testing.T) {
		...
		turnManager := world.Resources.TurnManager.(*turns.TurnManager)
		turnManager.TurnPhase = turns.TurnEnd
		initialTurnNumber := turnManager.TurnNumber

		sys := &TurnSystem{}
		err := sys.Update(world)
		require.NoError(t, err)

		assert.Equal(t, turns.PlayerTurn, turnManager.TurnPhase, "TurnEndからPlayerTurnへ遷移するべき")
		assert.Equal(t, initialTurnNumber+1, turnManager.TurnNumber, "ターン番号が増加するべき")
	})
}
...
func TestColdPlayerCanAct(t *testing.T) {
	...
	t.Run("CanEntityActの実装を確認", func(t *testing.T) {
		t.Parallel()
		world := testutil.InitTestWorld(t)
		...
		// CanEntityActの結果を確認
		canAct := turnManager.CanEntityAct(world, playerEntity, 100)
		t.Logf("CanEntityAct result: %v (TurnPhase=%v)", canAct, turnManager.TurnPhase)
	...
	t.Run("ConsumeActionPointsでAPが正しく消費される", func(t *testing.T) {
		...
		// アクションポイントを消費
		success := turnManager.ConsumeActionPoints(world, playerEntity, "攻撃", 100)
	...
	t.Run("完全なターンサイクルで冷えたプレイヤーが行動できる", func(t *testing.T) {
		...
		// 1. プレイヤーターンで行動可能
		t.Logf("初期状態: TurnPhase=%v, AP=%d", turnManager.TurnPhase, turnBased.AP.Current)
		canAct := turnManager.CanEntityAct(world, playerEntity, 100)
		assert.True(t, canAct, "初期状態で行動可能")

		// 2. 攻撃を実行（APを消費）
		turnManager.ConsumeActionPoints(world, playerEntity, "攻撃", 100)
		...
		err = turnManager.RestoreAllActionPoints(world)
		require.NoError(t, err)
	...
	t.Run("冷えた状態で敵に向かって移動すると攻撃が発動する", func(t *testing.T) {
		...
		// 行動可能であることを確認
		canAct := turnManager.CanEntityAct(world, playerEntity, 100)
		t.Logf("CanEntityAct結果: %v (TurnPhase=%v)", canAct, turnManager.TurnPhase)
