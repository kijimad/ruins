...
func TestSpawnSquadMember(t *testing.T) {
	...
	assert.True(t, member.HasComponent(world.Components.AIPolicy), "AIPolicyを持つ")
	assert.True(t, member.HasComponent(world.Components.AIState), "AIStateを持つ")
	...
	assert.Equal(t, gc.PlannerSquad, policy.Planner, "PlannerはSquad")
	assert.Equal(t, gc.MovementEscort, policy.Movement, "デフォルト移動ポリシーは護衛")
	assert.Equal(t, gc.CombatAttack, policy.CombatCurrent, "デフォルト戦闘ポリシーは攻撃")
...
func TestSetAIPolicy(t *testing.T) {
	t.Parallel()
	world := testutil.InitTestWorld(t)
...
func TestSetAIPolicy_移動だけ変更しても他のポリシーは変わらない(t *testing.T) {
	...
	policy := *world.Components.AIPolicy.Get(member).(*gc.AIPolicy)
	policy.Movement = gc.MovementPatrol
	err = SetAIPolicy(world, member, policy)
	require.NoError(t, err)

	current := world.Components.AIPolicy.Get(member).(*gc.AIPolicy)
	assert.Equal(t, gc.MovementPatrol, current.Movement, "移動ポリシーが変更された")
	assert.Equal(t, gc.CombatAttack, current.CombatCurrent, "戦闘ポリシーは変わらない")
}
