...
// PlaceFacility は据付アイテムをキューブの展開空間の coord へ据える。実体は削除せず、Item を外し Prop を
// 付けてロケーションをフィールドへ移す。マスが空いていなければ据えず error を返す。
...
// RemoveFacility は据えた設備をプレイヤーのバックパックへ戻す。実体は削除せず、Prop を外し Item を付けて
