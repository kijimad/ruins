...
func TestPlusMinusAmount(t *testing.T) {
	...
	require.NoError(t, ChangeStackableCount(world, "iron", 5))
	assert.Equal(t, 15, count(), "数量増加が正しく動作しない")

	require.NoError(t, ChangeStackableCount(world, "iron", -3))
	assert.Equal(t, 12, count(), "数量減少が正しく動作しない")

	// 大量追加テスト。上限は無い
	require.NoError(t, ChangeStackableCount(world, "iron", 1000))
...
func TestChangeStackableCount_未所持アイテムの操作(t *testing.T) {
	...
	t.Run("未所持で正の値を指定すると新規に生成される", func(t *testing.T) {
		...
		require.NoError(t, ChangeStackableCount(world, "healing_potion", 3))
