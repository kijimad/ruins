...
// 具体的な実装を提供する。中断可能なアクティビティシステムを採用し、
...
// ## 1. タイルレベル (マップ生成時)
//
//   - TileFloor: 通行可能 / TileWall: 通行不可
//   - mapplanner.PathFinder.IsWalkable() で判定
//   - 用途: 接続性検証、部屋配置、コリドー生成
//
// ## 2. エンティティレベル (実行時)
//
//   - 静的障害物: BlockPassコンポーネントを持つ固定物（壁・ドア）は常に通行不可
//   - キャラクター: プレイヤーは隊員と位置交換可能、それ以外はブロック
//   - CanMoveTo() で判定
//   - 用途: プレイヤー・AI移動時の衝突チェック
//
// マップ生成時にタイルからエンティティへ変換されるため、2層の一貫性は保たれる。
// TileWall → BlockPass付きエンティティ、TileFloor → 通行可能エンティティ。
//
// # アーキテクチャ
//
// ## 2層構造
