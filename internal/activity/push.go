...
func (pa *PushActivity) BuildActivity(_ ecs.Entity, world w.World) (*gc.Activity, error) {
	...
	if power <= 0 {
		return nil, fmt.Errorf("APが足りず押せません")
	}
	cost := query.PushCost(pa.cubeWeight(world))
	duration := consts.Turn((cost + power - 1) / power)
...
// cubeWeight はキューブの内装ステージの総重量を返す。内装リンク PortalConnection を持たなければ0。
func (pa *PushActivity) cubeWeight(world w.World) consts.Milligram {
	if !world.Components.PortalConnection.Has(pa.Cube) {
		return 0
	}
	return query.CubeWeight(world, world.Components.PortalConnection.Get(pa.Cube).Stage)
}
