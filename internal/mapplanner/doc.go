...
//   - タイル配列（[]oapi.Tile）、部屋情報、廊下情報、乱数生成器を含む
...
//
// いずれの場合も実行時はエンティティベースの通行可否判定（activity.CanMoveTo）を使用
//
// ## エラーハンドリング方針
//
// 配置プランナー（ItemPlanner, HostileNPCPlanner等）はエラーを2種類に分類する。
//   - データエラー（不正なsubtype、存在しないエントリ参照等）: 即座にerrorを返して中断する
//   - 配置失敗（位置が見つからない、重み選択で空等）: failCountを加算して処理を継続する。
//     配置数が目標に届かなくても許容する。マップ形状によって配置可能タイルが不足するケースが正常に起こり得るため
package mapplanner
