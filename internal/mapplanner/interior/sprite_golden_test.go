...
func TestGolden_BuildingHouse(t *testing.T) {
	...
	g.Assert(t, t.Name(), recordSeeds(t, func(seed uint64) (Site, []Placed) {
		return FurnishBuilding(testRaws(), seed, footprint, door, facHouse)
	}))
}
...
func TestGolden_BuildingStore(t *testing.T) {
	...
	g := goldie.New(t, goldie.WithNameSuffix(".png"))
	g.Assert(t, t.Name(), recordSeeds(t, func(seed uint64) (Site, []Placed) {
		return FurnishBuilding(testRaws(), seed, footprint, door, "store")
	}))
}
...
func TestGolden_BuildingClinic(t *testing.T) {
	...
	g := goldie.New(t, goldie.WithNameSuffix(".png"))
	g.Assert(t, t.Name(), recordSeeds(t, func(seed uint64) (Site, []Placed) {
		return FurnishBuilding(testRaws(), seed, footprint, door, facClinic)
	}))
}
...
func TestGolden_StagesHouse(t *testing.T) {
	...
	footprint := Rect{X: 0, Y: 0, W: prodFootprint, H: prodFootprint}
	door := Vec{X: prodFootprint / 2, Y: 0}
	site, stages := FurnishStages(testRaws(), 1, footprint, door, facHouse)
	g := goldie.New(t, goldie.WithNameSuffix(".png"))
...
func TestGolden_StagesStore(t *testing.T) {
	...
	footprint := Rect{X: 0, Y: 0, W: prodFootprint, H: prodFootprint}
	door := Vec{X: prodFootprint / 2, Y: 0}
	site, stages := FurnishStages(testRaws(), 1, footprint, door, "store")
	g := goldie.New(t, goldie.WithNameSuffix(".png"))
	g.Assert(t, t.Name(), recordStages(t, site, stages))
}
...
func TestGolden_StagesClinic(t *testing.T) {
	...
	footprint := Rect{X: 0, Y: 0, W: prodFootprint, H: prodFootprint}
	door := Vec{X: prodFootprint / 2, Y: 0}
	site, stages := FurnishStages(testRaws(), 1, footprint, door, facClinic)
	g := goldie.New(t, goldie.WithNameSuffix(".png"))
	g.Assert(t, t.Name(), recordStages(t, site, stages))
}
...
func TestGolden_RoomConvStore(t *testing.T) {
	...
	assertRoomGolden(t, room, "store", func(seed uint64) []Placed {
		return FillRoom(seed, room, contentByID(testRaws(), "convenience_store"))
	})
}
...
func TestGolden_RoomClinic(t *testing.T) {
	...
	assertRoomGolden(t, room, "clinic", func(seed uint64) []Placed {
		return FillRoom(seed, room, contentByID(testRaws(), "clinic"))
	})
}
...
func TestGolden_RoomHouse(t *testing.T) {
	...
	assertRoomGolden(t, room, "house", func(seed uint64) []Placed {
		return FillRoom(seed, room, contentByID(testRaws(), "house"))
	})
}
...
// drawLootMarker は床 loot の位置に item の文字を描き、拾える物があることを示す。中身は executor の抽選で
// 決まり計画層からは実アイテムを描けないので、位置だけを文字で示し in-game の field item と対応させる。
// golden は CPU 画像へ ASCII 専用の basicfont で描くため、日本語でなく item と表示する。可読性のため
// 暗い下地の上へ琥珀色で描く。
func drawLootMarker(img *image.RGBA, origin, pos Vec) {
	dx, dy := int(pos.X-origin.X)*cellPx, int(pos.Y-origin.Y)*cellPx
	const label = "item"
	const gh = 13 // basicfont.Face7x13 の行高
	d := &font.Drawer{Dst: img, Src: image.NewUniform(color.RGBA{R: 214, G: 164, B: 54, A: 255}), Face: basicfont.Face7x13}
	gw := d.MeasureString(label).Ceil()
	x0 := dx + (cellPx-gw)/2
	y0 := dy + (cellPx-gh)/2
	// 文字の外接矩形に1px 余白を足した暗い下地を敷き、床の上でも読めるようにする
	for yy := y0 - 1; yy < y0+gh+1 && yy < img.Bounds().Dy(); yy++ {
		for xx := x0 - 1; xx < x0+gw+1 && xx < img.Bounds().Dx(); xx++ {
			if xx >= 0 && yy >= 0 {
				img.SetRGBA(xx, yy, color.RGBA{R: 18, G: 18, B: 22, A: 235})
			}
		}
...
func drawPlaced(t *testing.T, img *image.RGBA, origin Vec, placed []Placed) {
	...
	for _, p := range placed {
		if p.Kind == KindLoot {
			// 床 loot は中身が executor の抽選で決まり、計画層からは実アイテムを描けない。位置に item の文字を
			// 描き、in-game でそこに拾える物が出ることと対応させる。写像を持たない Ref は描かない。
