...
func TestFurnish_施設種別ごとに決定的に内装を返す(t *testing.T) {
	...
	for _, fac := range []FacilityKind{"house", "store", "clinic", "office", "depot", "antique", "lab", "unknown"} {
		first := Furnish(testContents(), 3, footprint, door, fac)
		require.NotEmptyf(t, first, "%s は何か配置する", fac)
		require.Equalf(t, first, Furnish(testContents(), 3, footprint, door, fac), "%s は同じ引数で完全一致する", fac)
	}
}
...
func TestFurnish_密度と経年が建物ごとに変わる(t *testing.T) {
	...
	for seed := range uint64(30) {
		...
		n := 0
		for _, p := range Furnish(testContents(), seed, footprint, door, "store") {
...
func TestFacilityContent_seedで店の変種が変わる(t *testing.T) {
	...
	for seed := range uint64(30) {
		ids[testContents().facilityContent("store", seed).ID] = true
		assert.Equalf(t, "store", classifyRoom(Furnish(testContents(), seed, footprint, door, "store")), "seed=%d のどの変種も店に分類される", seed)
	}
...
func TestFurnish_家具は施設種別どおりに分類される(t *testing.T) {
	...
	for _, c := range cases {
		placed := Furnish(testContents(), 3, footprint, door, c.facility)
