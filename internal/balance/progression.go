...
// 想定装備トラックの仮説。能力は成長させず装備で生存力を伸ばす方針なので、進行に沿って装備由来の防御が
// 増えると仮定する。全枠最良の防御総和は約48で、想定プレイヤーは部分更新としてその3分の1程度を上限に置く。
// 装備更新は行動依存なので設計仮説。防御は per-hit を緩和し、敵の絶対ダメージが床には即死級でも想定には
// 多ターン消耗になる土俵を作る。見直す前提の値。
const (
