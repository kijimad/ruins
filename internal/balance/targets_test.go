...
func TestBaselineSnapshot_序盤戦闘(t *testing.T) {
	...
	require.NoError(t, err)
	assert.InDelta(t, 2.42, curve[0].PowerRatio, 0.05, "廃墟 day1 の戦力比。危険度1に帯電ダニが入り僅かに下がった")
	// day20 の床戦力比は後半専用敵の STR 強化で下がった。床はハードモードの下限で、主基準は想定の死亡確率。
	assert.InDelta(t, 0.67, curve[19].PowerRatio, 0.05, "廃墟 day20 の床戦力比。後半専用敵の強化で目標を下回るハードモード")
...
func TestBaselineSnapshot_探索収入(t *testing.T) {
	t.Parallel()
	master := loadTestMaster(t)
	assert.Equal(t, 46.0, ExpectedNetLootValue(master, "ruins_area", 8), "危険度8の廃墟で拾える1個あたりの店売り期待手取り。額面に売却倍率を掛けた値")
	assert.Equal(t, 2945.0, ExpectedRunLootIncome(master, "ruins_area", 5), "廃墟5層探索の店売り期待収入")
...
func TestBaselineSnapshot_ビルドブレ(t *testing.T) {
	...
	assert.InDelta(t, 2.05, stages[2].MaxSpread, 0.2, "終盤のブレ幅")
}
