...
// Params はバランス導出が参照する全つまみを1つに集約したパラメータベクトル。
// 導出関数はこの値の純関数として書く。感度・交換レート・探索はこの成分を摂動する
// 汎用機械になり、つまみを増やすときは成分とレジストリの追加だけで済む。
//
// 成分は2種類ある。ゲーム定数を写した絶対値と、raw.toml やクエリ由来の集計へ掛ける倍率。
// 絶対値の初期値は DefaultParams だけがゲーム定数から取り込み、接点を1箇所に限る。
type Params struct {
	// 時間。日換算メトリクスの共通分母
	TurnsPerDay float64
