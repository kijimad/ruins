...
## ゴールデンテスト（フレーク源を増やさない）

- 新規・変更ゴールデンが**画像のピクセル一致検証**を増やしていないか。world やメニューの内容は、画像でなく決定的なテキスト/構造化 golden で assert する。画像は目視用に保存し比較しない。全体設計は設計 doc 74。
- ピクセル・トレランス比較を新規に足すのは、見た目そのものが不変条件の UI 部品に限る。world/ECS を描く画面をピクセル golden にしない。
- golden 化する列挙が決定的順序か。ark の反復順に依存せず、座標や `entity.ID()`・`query.SortEntities` でソートしてから並べているか。
