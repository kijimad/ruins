...
`keybind.Binding` の表は同じ Action へ複数キーを割り当てられるぶん、キーの意味の一貫性は人が守る。同種の逸脱が繰り返し出ている。

- **閉じるキーは Esc と、その画面を開いたキーだけにする**。俯瞰図は N で開くので N で閉じるトグルは正当。見回しの x 閉じ、俯瞰図の M 閉じは、x が調べる・M がメニューを開く意味と衝突する残骸だった。他所で別の意味を持つキーを閉じるに流用しない。
- **1つの意味に1つのキー**を崩さない。あるキーが文脈をまたいで別の Action を指すと、体験が壊れる。追加・変更時は、そのキーが他の表で持つ意味と衝突しないか確認する。
