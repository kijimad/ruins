---
name: add-component
description: ruins に新しい ECS コンポーネントを追加する手順。型定義・登録表・生成・serde除外・クエリ設計・テスト・System登録の漏れを防ぐ。コンポーネント・マーカー・System を増やすときに使う
---
...
7. **System を新規追加した場合は `InitializeSystems` へ登録する**
   - 呼び出しリストへ足すだけでは毎フレーム黙ってスキップされる。登録漏れは実行時エラーにならず気づきにくい。
