...
- ロジックやコメントは日本語で書く。
- **コメントはデフォルトオフ。** 書いてよいのは関数の doc コメント、パッケージコメント、そして関数内で本当に注意が要る箇所だけ。関数内の手順ナラティブ、setup の言い換え、見ればわかる自明な説明は書かない。関数 doc は非自明な理由を簡潔に。厚く書いて後で削る往復を繰り返さない。
- **例外: doc.go のパッケージコメントはしっかり書く。** package の説明・使い分け・責務・仕様を厚く書いてよい。1行に絞る対象は関数 doc と関数内コメントで、package doc は別。
- **コード上の識別子は訳さずその名で呼ぶ。** 型・関数名はコメントでもその識別子で書く。`saveEnvelope` を「封筒」と訳すと奇妙な言い回しになり、コメントとコードの対応も切れる。ドメイン概念を日本語へ訳す下の規約とは別で、識別子はそのまま使う。
...
- **借用語・造語をコメントに混ぜない。日本語で自己完結させる。** 翻案元の英単語や造語をそのままコメントへ持ち込まない。日本語の定着語へ訳す。何度も再発している。悪い例と直し: `need`→`生理値`（疲労・空腹）、`materialize`→`組み立てる`/`導出する`、`funnel`→`集約`/`経路`、`capacity`→`身体機能の値`、`master 乗数`→`全体乗数`、`保存 condition`/`need condition`→`保存された不調`/`量から導いた不調`。`Hediff`・`capacity` 等の RimWorld/CDDA 用語は設計 doc の比較節でだけ使い、地の文とコード注釈では使わない。既存の日本語コメント語彙（`不調`・`意識`・`量`・`生理値`）に合わせる。
- **同じ説明を複数箇所に書かない。** 設計の根拠は定数ブロックか型 doc の1箇所に置き、ヘルパー doc や switch の case コメントで繰り返さない。式の言い換えはヘルパー名に語らせる。重複した説明は片方だけ改定されて食い違う。
- **比喩でなく直接の語で書く。** 値の書き込みを「焼く」、探索矩形を「窓」のように呼ばず、「書き込む」「再計算範囲」と操作や対象をそのまま指す語で書く。比喩は読み手に翻訳を強い、ゲーム内の実物とも紛れる。
...
- **enum → 表示名・msgid・色などの純粋な写像はユニットテストしない。** 網羅性は exhaustive linter が `default` の無い switch で守るので、実装のリテラルを写経する文字列・色の一致テストは独立した検証にならず、綴りや色を変えるたびに追随するだけの死んだテストになる。カバレッジ目的でこの種のテストを足さない。検証するのは分岐・閾値・境界・契約のロジックのほう。例: 気温表示色は「下限未満は青、上限超過は赤、境界ちょうどは白」の分岐は固定するが、色の RGBA 値そのものは固定しない。悪化メッセージは「低体温以外は空、重症度の両端は空」という契約を固定するが、文言そのものは固定しない。
