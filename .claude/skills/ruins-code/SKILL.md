...
- ロジックやコメントは日本語で書く。
- **コメントはデフォルトオフ。** 関数・パッケージの doc コメントと、本当に注意が要る箇所のインラインコメントだけ書く。自明な説明・手順の言い換えは書かない。関数 doc は簡潔に。ただし doc.go のパッケージコメントは説明・使い分け・責務・仕様をしっかり書く。
