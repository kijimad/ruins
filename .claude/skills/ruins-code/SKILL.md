...
**決定的な状態を assert し、画像のピクセル一致検証はしない。** ebiten/xvfb の描画ピクセル比較は環境依存でフレーキーな一方、トレランスの盲点で構造変化を見逃す。退行は画像でなく状態のテキスト/構造化データで捉える。移行の全体設計は設計 doc 74。手法は対象で選ぶ:

- **world/ECS を描く state** → タイルグリッド + エンティティを決定的順(座標→ID)でテキスト化して `.txt`/`.json` golden。`maptemplate.FormatResolvedMap` と `mapplanner.Snapshot`(JSON) が手本。
- **メニュー等の論理内容** → fetchProps 等のビューモデルを JSON golden へ直列化する。手書きの文字列整形はしない。非決定値のフィールドは `json:"-"` で除外し、表示順のソートは fetchProps 本体で行う。ウィジェットツリーの汎用ダンプはしない。
