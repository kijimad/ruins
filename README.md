...
|---|---|---|
| 隊員アイテム運搬システム | 0/11 |  |
| ECSエンジンの goecs から Ark への移行 | 30/33 |  |
| CI ベストプラクティス施策の提案（安全性・開発効率・カバレッジ精度） | 4/14 |  |
| マクロ移動（キャラバンのルート網踏破）の実装設計 | 0/13 |  |
| マクロ移動の廃止 ── 出口選択型ラン構造（Hades/DD型） | 0/5 |  |
| 高速移動（走り）の実装設計 | 13/26 |  |
| ゲームパフォーマンステスト戦略 | 9/11 |  |
| シームレスワールドの実装 ── スライディング帯によるチャンクストリーミング | 8/9 |  |
| GitHub Actions CI のキャッシュ最適化（Docker / Go） | 3/4 |  |
| シームレスワールド Phase 7: 永続ステージと往復 | 9/18 |  |
| 型ユーティリティによる堅牢化・利便化の調査と適用方針 | 0/9 |  |
